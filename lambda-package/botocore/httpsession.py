...
try:
    from lambda.vendor.urllib3.util.ssl_ import DEFAULT_CIPHERS
except ImportError:
...
class URLLib3Session:
    ...
    def __init__(
        ...
        self._pool_classes_by_scheme = {
            'http': lambda.vendor.botocore.awsrequest.AWSHTTPConnectionPool,
            'https': lambda.vendor.botocore.awsrequest.AWSHTTPSConnectionPool,
        }
        if timeout is None:
    ...
    def send(self, request):
        try:
            ...
            http_response = lambda.vendor.botocore.awsrequest.AWSResponse(
                request.url,

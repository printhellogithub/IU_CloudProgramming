...
import lambda.vendor.botocore.compat
import lambda.vendor.botocore.configloader
from lambda.vendor.botocore import UNSIGNED
...
from lambda.vendor.botocore.tokens import SSOTokenProvider
from lambda.vendor.botocore.useragent import register_feature_id, register_feature_ids
...
class Credentials:
    ...
    def _normalize(self):
        ...
        self.access_key = lambda.vendor.botocore.compat.ensure_unicode(self.access_key)
        self.secret_key = lambda.vendor.botocore.compat.ensure_unicode(self.secret_key)
...
class RefreshableCredentials(Credentials):
    ...
    def _normalize(self):
        self._access_key = lambda.vendor.botocore.compat.ensure_unicode(self._access_key)
        self._secret_key = lambda.vendor.botocore.compat.ensure_unicode(self._secret_key)
...
class ProcessProvider(CredentialProvider):
    ...
    def _retrieve_credentials_using(self, credential_process):
        ...
        parsed = lambda.vendor.botocore.compat.json.loads(stdout.decode('utf-8'))
...
class SharedCredentialProvider(CredentialProvider):
    ...
    def __init__(self, creds_filename, profile_name=None, ini_parser=None):
        ...
        if ini_parser is None:
            ini_parser = lambda.vendor.botocore.configloader.raw_config_parse
...
class ConfigProvider(CredentialProvider):
    ...
    def __init__(self, config_filename, profile_name, config_parser=None):
        ...
        if config_parser is None:
            config_parser = lambda.vendor.botocore.configloader.load_config
...
class LoginCredentialFetcher:
    ...
    def refresh_credentials(self):
        ...
        config = botocore.config.Config(
            signature_version=lambda.vendor.botocore.UNSIGNED,
        )

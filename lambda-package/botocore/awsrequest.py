...
import lambda.vendor.urllib3.util
from lambda.vendor.urllib3.connection import HTTPConnection, VerifiedHTTPSConnection
from lambda.vendor.urllib3.connectionpool import HTTPConnectionPool, HTTPSConnectionPool

import lambda.vendor.botocore.utils
...
from lambda.vendor.botocore.exceptions import UnseekableStreamError
...
class AWSConnection:
    ...
    def _send_output(self, message_body=None, *args, **kwargs):
        ...
        if self._expect_header_set:
            ...
            if lambda.vendor.urllib3.util.wait_for_read(self.sock, 1):
...
def prepare_request_dict(
    ...
    if r['query_string']:
        ...
        # breaking chainges.
        percent_encode_sequence = lambda.vendor.botocore.utils.percent_encode_sequence
...
class AWSRequestPreparer:
    ...
    def _determine_content_length(self, body):
        return lambda.vendor.botocore.utils.determine_content_length(body)
...
class AWSResponse:
    ...
    def text(self):
        ...
        """
        encoding = lambda.vendor.botocore.utils.get_encoding_from_headers(self.headers)
        if encoding:

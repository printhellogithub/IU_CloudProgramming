...
import lambda
from lambda.vendor.botocore import translate
from lambda.vendor.botocore import retryhandler
from lambda.vendor.botocore import paginate
from lambda.vendor.botocore import monitoring
from lambda.vendor.botocore import handlers.vendor.botocore.client
import lambda.vendor.botocore.configloader
import lambda.vendor.botocore.credentials
import lambda.vendor.botocore.tokens
from lambda.vendor.botocore import (
    waiter,
)
...
from lambda.vendor.botocore.configprovider import (
...
from lambda.vendor.botocore.context import get_context, with_current_context
from lambda.vendor.botocore.errorfactory import ClientExceptionsFactory
...
from lambda.vendor.botocore.hooks import (
    EventAliaser,
    HierarchicalEmitter,
    first_non_none_response,
)
from lambda.vendor.botocore.loaders import create_loader
from lambda.vendor.botocore.model import ServiceModel
from lambda.vendor.botocore.parsers import ResponseParserFactory
from lambda.vendor.botocore.plugin import get_botocore_plugins, load_client_plugins
from lambda.vendor.botocore.regions import EndpointResolver
from lambda.vendor.botocore.useragent import UserAgentString, register_feature_id
from lambda.vendor.botocore.utils import (
    EVENT_ALIASES,
    IMDSRegionProvider,
    validate_region_name,
)
from lambda.vendor.botocore import UNSIGNED, __version__, invoke_initializers
...
class Session:
    ...
    def _create_token_resolver(self):
        return lambda.vendor.botocore.tokens.create_token_resolver(self)
    ...
    def _create_credential_resolver(self):
        return lambda.vendor.botocore.credentials.create_credential_resolver(
        ...
        **entire** config file.
        ...
        if self._config is None:
            try:
                ...
                self._config = lambda.vendor.botocore.configloader.load_config(config_file)
            ...
            try:
                ...
                cred_profiles = lambda.vendor.botocore.configloader.raw_config_parse(
                    cred_file
                )
    ...
    def set_credentials(
        ...
        """
        self._credentials = lambda.vendor.botocore.credentials.Credentials(
    ...
    def create_client(
        ...
        if config is not None and config.signature_version is UNSIGNED:
            ...
            credentials = lambda.vendor.botocore.credentials.Credentials(
        ...
        client_creator = lambda.vendor.botocore.client.ClientCreator(
            loader,
            endpoint_resolver,
            self.user_agent(),
            event_emitter,

...
import lambda.vendor.botocore.httpsession
...
from lambda.vendor.botocore.plugin import (
    PluginContext,
...
def has_header(header_name, headers):
    ...
    if header_name is None:
        return False
    elif isinstance(headers, lambda.vendor.botocore.awsrequest.HeadersDict):
...
class IMDSFetcher:
    ...
    def __init__(
        ...
        self._session = lambda.vendor.botocore.httpsession.URLLib3Session(
    ...
    def _fetch_metadata_token(self):
        ...
        request = lambda.vendor.botocore.awsrequest.AWSRequest(
...
class ContainerMetadataFetcher:
    ...
    def __init__(self, session=None, sleep=time.sleep):
        if session is None:
            session = lambda.vendor.botocore.httpsession.URLLib3Session(
    ...
    def _get_response(self, full_url, headers, timeout):
        try:
            AWSRequest = lambda.vendor.botocore.awsrequest.AWSRequest

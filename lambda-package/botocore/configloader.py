...
def multi_file_load_config(*filenames):
    ...
    for filename in filenames:
        ...
        except lambda.vendor.botocore.exceptions.ConfigNotFound:
            continue
...
def raw_config_parse(config_filename, parse_subsections=True):
    ...
    if path is not None:
        ...
        if not os.path.isfile(path):
            raise lambda.vendor.botocore.exceptions.ConfigNotFound(path=_unicode_path(path))
        cp = configparser.RawConfigParser()
        try:
            cp.read([path])
        except (configparser.Error, UnicodeDecodeError) as e:
            raise lambda.vendor.botocore.exceptions.ConfigParseError(

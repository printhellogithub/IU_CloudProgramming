...
import lambda.vendor.botocore.session
...
from lambda.vendor.botocore.compat import urlsplit
from lambda.vendor.botocore.config import Config
from lambda.vendor.botocore.exceptions import InvalidConfigError, NoCredentialsError
from lambda.vendor.botocore.utils import ArnParser, InvalidArnException

from lambda.vendor.s3transfer.constants import FULL_OBJECT_CHECKSUM_ARGS, MB
from lambda.vendor.s3transfer.exceptions import TransferNotDoneError
from lambda.vendor.s3transfer.futures import BaseTransferFuture, BaseTransferMeta
from lambda.vendor.s3transfer.manager import TransferManager
from lambda.vendor.s3transfer.utils import (
    CallArgs,
    OSUtils,
...
class BotocoreCRTRequestSerializer(BaseCRTRequestSerializer):
    ...
    def _make_fake_http_response(self, request, **kwargs):
        return lambda.vendor.botocore.awsrequest.AWSResponse(
            None,
            200,
            {},
            FakeRawResponse(b""),
    ...
    def _translate_crt_s3_response_error(self, s3_response_error):
        ...
        response_dict = {
            'headers': lambda.vendor.botocore.awsrequest.HeadersDict(headers),

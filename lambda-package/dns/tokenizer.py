...
class UngetBufferFull(lambda.vendor.dns.exception.DNSException):
...
class Tokenizer:
    ...
    def __init__(
        ...
        if idna_codec is None:
            self.idna_codec: lambda.vendor.dns.name.IDNACodec = lambda.vendor.dns.name.IDNA_2003
        else:
    ...
    def get(self, want_leading: bool = False, want_comment: bool = False) -> Token:
        ...
        while True:
            ...
            if c == "" or c in self.delimiters:
                ...
                raise lambda.vendor.dns.exception.SyntaxError("newline in quoted string")
            elif c == "\\":
                #
        ...
        if token == "" and ttype != QUOTED_STRING:
            if self.multiline:
                raise lambda.vendor.dns.exception.SyntaxError("unbalanced parentheses")
            ttype = EOF
    ...
    def get_int(self, base: int = 10) -> int:
        ...
        if not token.is_identifier():
            raise lambda.vendor.dns.exception.SyntaxError("expecting an identifier")
        if not token.value.isdigit():
            raise lambda.vendor.dns.exception.SyntaxError("expecting an integer")
    ...
    def get_uint8(self) -> int:
        ...
        if value < 0 or value > 255:
            raise lambda.vendor.dns.exception.SyntaxError(f"{value} is not an unsigned 8-bit integer")
        return value
    ...
    def get_string(self, max_length: int | None = None) -> str:
        ...
        if not (token.is_identifier() or token.is_quoted_string()):
            raise lambda.vendor.dns.exception.SyntaxError("expecting a string")
        if max_length and len(token.value) > max_length:
            raise lambda.vendor.dns.exception.SyntaxError("string too long")
        return token.value
    ...
    def concatenate_remaining_identifiers(self, allow_empty: bool = False) -> str:
        ...
        if not (allow_empty or s):
            raise lambda.vendor.dns.exception.SyntaxError("expecting another identifier")
        return s
    ...
    def as_name(
        ...
        if not token.is_identifier():
            raise lambda.vendor.dns.exception.SyntaxError("expecting an identifier")
        name = lambda.vendor.dns.name.from_text(token.value, origin, self.idna_codec)

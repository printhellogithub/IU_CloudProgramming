...
import lambda.vendor.dns.asyncquery
...
import lambda.vendor.dns.resolver  # lgtm[py/import-and-import-from]
import lambda.vendor.dns.reversename

# import some resolver symbols for brevity
from lambda.vendor.dns.resolver import NXDOMAIN, NoAnswer, NoRootSOA, NotAbsolute

# for indentation purposes below
_udp = lambda.vendor.dns.asyncquery.udp
_tcp = lambda.vendor.dns.asyncquery.tcp
...
class Resolver(dns.resolver.BaseResolver):
    ...
    async def canonical_name(self, name: lambda.vendor.dns.name.Name | str) -> lambda.vendor.dns.name.Name:
    ...
    async def try_ddr(self, lifetime: float = 5.0) -> None:
        ...
        *lifetime*, a float, is the maximum time to spend attempting DDR.  The default
        ...
        try:
            ...
            nameservers = await lambda.vendor.dns._ddr._get_nameservers_async(answer, timeout)
...
async def resolve_name(
    name: lambda.vendor.dns.name.Name | str, family: int = socket.AF_UNSPEC, **kwargs: Any
) -> dns.resolver.HostAnswers:
...
async def canonical_name(name: lambda.vendor.dns.name.Name | str) -> lambda.vendor.dns.name.Name:
    """Determine the canonical name of *name*.
...
async def zone_for_name(
    ...
    if isinstance(name, str):
        name = lambda.vendor.dns.name.from_text(name, lambda.vendor.dns.name.root)
    if resolver is None:
    ...
    while True:
        ...
        try:
            name = name.parent()
        except lambda.vendor.dns.name.NoParent:  # pragma: no cover
            raise NoRootSOA


async def make_resolver_at(
    where: lambda.vendor.dns.name.Name | str,
    port: int = 53,
    family: int = socket.AF_UNSPEC,
    resolver: Resolver | None = None,
) -> Resolver:

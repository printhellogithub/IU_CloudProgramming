...
class TransactionManager:
    ...
    def get_class(self) -> lambda.vendor.dns.rdataclass.RdataClass:
        """The class of the transaction manager."""
        raise NotImplementedError  # pragma: no cover

    def from_wire_origin(self) -> lambda.vendor.dns.name.Name | None:
...
class DeleteNotExact(lambda.vendor.dns.exception.DNSException):
    """Existing data did not match data specified by an exact delete."""


class ReadOnly(lambda.vendor.dns.exception.DNSException):
    """Tried to write to a read-only transaction."""


class AlreadyEnded(lambda.vendor.dns.exception.DNSException):
...
CheckPutRdatasetType = Callable[
    ["Transaction", lambda.vendor.dns.name.Name, dns.rdataset.Rdataset], None
]
CheckDeleteRdatasetType = Callable[
    ["Transaction", lambda.vendor.dns.name.Name, dns.rdatatype.RdataType, dns.rdatatype.RdataType],
    None,
]
CheckDeleteNameType = Callable[["Transaction", lambda.vendor.dns.name.Name], None]
...
class Transaction:
    ...
    def get(
        self,
        name: lambda.vendor.dns.name.Name | str | None,
        ...
        covers = lambda.vendor.dns.rdatatype.RdataType.make(covers)
    ...
    def name_exists(self, name: lambda.vendor.dns.name.Name | str) -> bool:
    ...
    def update_serial(
        ...
        value: int = 1,
        relative: bool = True,
        name: lambda.vendor.dns.name.Name = lambda.vendor.dns.name.empty,
    ) -> None:
    ...
    def iterate_names(self) -> Iterator[lambda.vendor.dns.name.Name]:
    ...
    def _add(self, replace, args):
        ...
        try:
            ...
            if isinstance(arg, str):
                arg = lambda.vendor.dns.name.from_text(arg, None)
            if isinstance(arg, lambda.vendor.dns.name.Name):
                name = arg

...
import lambda.vendor.dns.dnssectypes
...
class BadSigTime(lambda.vendor.dns.exception.DNSException):
...
class RRSIG(lambda.vendor.dns.rdata.Rdata):
    """RRSIG record"""
    ...
    def __init__(
        ...
        self.algorithm = lambda.vendor.dns.dnssectypes.Algorithm.make(algorithm)
    ...
    def to_text(self, origin=None, relativize=True, **kw):
        return (
            f"{lambda.vendor.dns.rdatatype.to_text(self.type_covered)} "
            ...
            f"{lambda.vendor.dns.rdata._base64ify(self.signature, **kw)}"  # pyright: ignore
        )
    ...
    def from_text(
        ...
        type_covered = lambda.vendor.dns.rdatatype.from_text(tok.get_string())
        algorithm = lambda.vendor.dns.dnssectypes.Algorithm.from_text(tok.get_string())
        labels = tok.get_int()

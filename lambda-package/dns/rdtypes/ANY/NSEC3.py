...
class Bitmap(lambda.vendor.dns.rdtypes.util.Bitmap):
    type_name = "NSEC3"


@dns.immutable.immutable
class NSEC3(lambda.vendor.dns.rdata.Rdata):
    """NSEC3 record"""
    ...
    def next_name(self, origin=None):
        return lambda.vendor.dns.name.from_text(self._next_text(), origin)

...
class OPT(lambda.vendor.dns.rdata.Rdata):
    """OPT record"""
    ...
    def __init__(self, rdclass, rdtype, options):
        ...
        *options*, a tuple of ``bytes``
        ...
        def as_option(option):
            if not isinstance(option, lambda.vendor.dns.edns.Option):
    ...
    def from_wire_parser(cls, rdclass, rdtype, parser, origin=None):
        ...
        while parser.remaining() > 0:
            ...
            with parser.restrict_to(olen):
                opt = lambda.vendor.dns.edns.option_from_wire_parser(otype, parser)
            options.append(opt)

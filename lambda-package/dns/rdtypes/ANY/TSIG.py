...
class TSIG(lambda.vendor.dns.rdata.Rdata):
    """TSIG record"""
    ...
    def __init__(
        ...
        *other*, a ``bytes``
        ...
        self.error = lambda.vendor.dns.rcode.Rcode.make(error)
        self.other = self._as_bytes(other)

    def to_text(self, origin=None, relativize=True, **kw):
        algorithm = self.algorithm.choose_relativity(origin, relativize)
        error = lambda.vendor.dns.rcode.to_text(self.error, True)
        text = (
            f"{algorithm} {self.time_signed} {self.fudge} "
            + f"{len(self.mac)} {lambda.vendor.dns.rdata._base64ify(self.mac, 0)} "
            + f"{self.original_id} {error} {len(self.other)}"
        )
        if self.other:
            text += f" {lambda.vendor.dns.rdata._base64ify(self.other, 0)}"
        return text
    ...
    def from_text(
        ...
        error = lambda.vendor.dns.rcode.from_text(tok.get_string())

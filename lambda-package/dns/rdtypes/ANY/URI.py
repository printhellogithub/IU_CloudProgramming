...
class URI(lambda.vendor.dns.rdata.Rdata):
    """URI record"""
    ...
    def __init__(self, rdclass, rdtype, priority, weight, target):
        ...
        if len(self.target) == 0:
            raise lambda.vendor.dns.exception.SyntaxError("URI target cannot be empty")
    ...
    def from_text(
        ...
        if not (target.is_quoted_string() or target.is_identifier()):
            raise lambda.vendor.dns.exception.SyntaxError("URI target must be a string")
    ...
    def from_wire_parser(cls, rdclass, rdtype, parser, origin=None):
        ...
        if len(target) == 0:
            raise lambda.vendor.dns.exception.FormError("URI target may not be empty")

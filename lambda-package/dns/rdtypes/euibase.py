...
class EUIBase(lambda.vendor.dns.rdata.Rdata):
    """EUIxx record"""
    ...
    def to_text(self, origin=None, relativize=True, **kw):
        return lambda.vendor.dns.rdata._hexify(self.eui, chunksize=2, separator=b"-", **kw)
    ...
    def from_text(
        ...
        for i in range(2, cls.byte_len * 3 - 1, 3):
            if text[i] != "-":
                raise lambda.vendor.dns.exception.SyntaxError(f"Dash expected at position {i}")
        text = text.replace("-", "")
        try:
            data = binascii.unhexlify(text.encode())
        except (ValueError, TypeError) as ex:
            raise lambda.vendor.dns.exception.SyntaxError(f"Hex decoding error: {str(ex)}")

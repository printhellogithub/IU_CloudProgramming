...
class NSAP(lambda.vendor.dns.rdata.Rdata):
    """NSAP record."""
    ...
    def from_text(
        ...
        if address[0:2] != "0x":
            raise lambda.vendor.dns.exception.SyntaxError("string does not start with 0x")
        address = address[2:].replace(".", "")
        if len(address) % 2 != 0:
            raise lambda.vendor.dns.exception.SyntaxError("hexstring has odd length")

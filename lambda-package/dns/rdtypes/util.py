...
class Gateway:
    ...
    def __init__(self, type: Any, gateway: str | lambda.vendor.dns.name.Name | None = None):
    ...
    def _check(self):
        if self.type == 0:
            ...
            lambda.vendor.dns.ipv4.inet_aton(self.gateway)
        elif self.type == 2:
            # check that it's OK
            assert isinstance(self.gateway, str)
            lambda.vendor.dns.ipv6.inet_aton(self.gateway)
        elif self.type == 3:
            if not isinstance(self.gateway, lambda.vendor.dns.name.Name):
    ...
    def to_text(self, origin=None, relativize=True):
        if self.type == 0:
            ...
            assert isinstance(self.gateway, lambda.vendor.dns.name.Name)
    ...
    def to_wire(self, file, compress=None, origin=None, canonicalize=False):
        if self.type == 0:
            ...
            assert isinstance(self.gateway, str)
            file.write(lambda.vendor.dns.ipv4.inet_aton(self.gateway))
        elif self.type == 2:
            assert isinstance(self.gateway, str)
            file.write(lambda.vendor.dns.ipv6.inet_aton(self.gateway))
        elif self.type == 3:
            assert isinstance(self.gateway, lambda.vendor.dns.name.Name)
    ...
    def from_wire_parser(cls, gateway_type, parser, origin=None):
        if gateway_type == 0:
            ...
            gateway = lambda.vendor.dns.ipv4.inet_ntoa(parser.get_bytes(4))
        elif gateway_type == 2:
            gateway = lambda.vendor.dns.ipv6.inet_ntoa(parser.get_bytes(16))
        elif gateway_type == 3:
            gateway = parser.get_name(origin)
        else:
            raise lambda.vendor.dns.exception.FormError(cls._invalid_type(gateway_type))
        return cls(gateway_type, gateway)
    ...
    def to_text(self) -> str:
        ...
        for window, bitmap in self.windows:
            ...
            for i, byte in enumerate(bitmap):
                for j in range(0, 8):
                    if byte & (0x80 >> j):
                        rdtype = lambda.vendor.dns.rdatatype.RdataType.make(window * 256 + i * 8 + j)
                        bits.append(lambda.vendor.dns.rdatatype.to_text(rdtype))
            text += " " + " ".join(bits)
        return text

    @classmethod
    def from_text(cls, tok: "lambda.vendor.dns.tokenizer.Tokenizer") -> "Bitmap":
        rdtypes = []
        for token in tok.get_remaining():
            rdtype = lambda.vendor.dns.rdatatype.from_text(token.unescape().value)
            if rdtype == 0:
                raise lambda.vendor.dns.exception.SyntaxError(f"{cls.type_name} with bit 0")
            rdtypes.append(rdtype)
        return cls.from_rdtypes(rdtypes)

    @classmethod
    def from_rdtypes(cls, rdtypes: List[lambda.vendor.dns.rdatatype.RdataType]) -> "Bitmap":

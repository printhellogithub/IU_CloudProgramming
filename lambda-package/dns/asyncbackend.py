...
from lambda.vendor.dns._asyncbackend import (  # noqa: F401  lgtm[py/unused-import]
    Backend,
    DatagramSocket,
    Socket,
    StreamSocket,
...
class AsyncLibraryNotFoundError(lambda.vendor.dns.exception.DNSException):
    pass
...
def get_backend(name: str) -> Backend:
    ...
    *name*, a ``str``, the name of the backend.  Currently the "trio"
    ...
    if backend:
        return backend
    if name == "trio":
        import lambda.vendor.dns._trio_backend

        backend = lambda.vendor.dns._trio_backend.Backend()
    elif name == "asyncio":
        import lambda.vendor.dns._asyncio_backend

        backend = lambda.vendor.dns._asyncio_backend.Backend()
    else:

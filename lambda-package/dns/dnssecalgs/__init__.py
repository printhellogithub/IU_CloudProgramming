...
from lambda.vendor.dns.dnssecalgs.base import GenericPrivateKey
from lambda.vendor.dns.dnssectypes import Algorithm
from lambda.vendor.dns.exception import UnsupportedAlgorithm
from lambda.vendor.dns.rdtypes.ANY.DNSKEY import DNSKEY

# pyright: reportPossiblyUnboundVariable=false

if lambda.vendor.dns._features.have("dnssec"):
    from lambda.vendor.dns.dnssecalgs.dsa import PrivateDSA, PrivateDSANSEC3SHA1
    from lambda.vendor.dns.dnssecalgs.ecdsa import PrivateECDSAP256SHA256, PrivateECDSAP384SHA384
    from lambda.vendor.dns.dnssecalgs.eddsa import PrivateED448, PrivateED25519
    from lambda.vendor.dns.dnssecalgs.rsa import (
        PrivateRSAMD5,
        PrivateRSASHA1,
...
AlgorithmPrefix = bytes | lambda.vendor.dns.name.Name | None
...
def get_algorithm_cls_from_dnskey(dnskey: DNSKEY) -> Type[GenericPrivateKey]:
    ...
    *dnskey*, a ``DNSKEY`` to get Algorithm class for.
    ...
    if dnskey.algorithm == Algorithm.PRIVATEDNS:
        prefix, _ = lambda.vendor.dns.name.from_wire(dnskey.key, 0)
    ...
    *oid*: an optional BER-encoded `bytes` for PRIVATEOID algorithms.
    ...
    if algorithm == Algorithm.PRIVATEDNS:
        ...
        if isinstance(name, str):
            name = lambda.vendor.dns.name.from_text(name)
        prefix = name

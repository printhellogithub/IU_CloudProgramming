...
class BaseQuicConnection:
    def __init__(
        ...
        if manager is not None and manager.is_h3():
            ...
            self._h3_conn = None
        self._af = lambda.vendor.dns.inet.af_for_address(address)
        self._peer = lambda.vendor.dns.inet.low_level_address_tuple((address, port))
...
class BaseQuicManager:
    def __init__(
        ...
        if conf is None:
            ...
            if verify_path is not None:
                cafile, capath = lambda.vendor.dns._tls_util.convert_verify_to_cafile_and_capath(
                    verify_path
                )
    ...
    def _connect(
        ...
        lladdress = lambda.vendor.dns.inet.low_level_address_tuple((address, port))

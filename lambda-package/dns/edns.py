...
class OptionType(lambda.vendor.dns.enum.IntEnum):
...
class Option:
    ...
    @classmethod
    def from_wire_parser(cls, otype: OptionType, parser: "lambda.vendor.dns.wire.Parser") -> "Option":
...
class GenericOption(Option):  # lgtm[py/missing-equals]
    ...
    def __init__(self, otype: OptionType | str, data: bytes | str):
        ...
        self.data = lambda.vendor.dns.rdata.Rdata._as_bytes(data, True)
    ...
    def from_wire_parser(
        cls, otype: OptionType | str, parser: "lambda.vendor.dns.wire.Parser"
    ) -> Option:
...
class ECSOption(Option):  # lgtm[py/missing-equals]
    ...
    def __init__(self, address: str, srclen: int | None = None, scopelen: int = 0):
        ...
        *scopelen*, an ``int``, the scope prefix length.  This value
        ...
        if af == socket.AF_INET6:
            ...
            if srclen is None:
                srclen = 56
            address = lambda.vendor.dns.rdata.Rdata._as_ipv6_address(address)
            srclen = lambda.vendor.dns.rdata.Rdata._as_int(srclen, 0, 128)
            scopelen = lambda.vendor.dns.rdata.Rdata._as_int(scopelen, 0, 128)
        elif af == socket.AF_INET:
            self.family = 1
            if srclen is None:
                srclen = 24
            address = lambda.vendor.dns.rdata.Rdata._as_ipv4_address(address)
            srclen = lambda.vendor.dns.rdata.Rdata._as_int(srclen, 0, 32)
            scopelen = lambda.vendor.dns.rdata.Rdata._as_int(scopelen, 0, 32)
        ...
        addrdata = lambda.vendor.dns.inet.inet_pton(af, address)
    ...
    def from_wire_parser(
        ...
        if family == 1:
            pad = 4 - addrlen
            addr = lambda.vendor.dns.ipv4.inet_ntoa(prefix + b"\x00" * pad)
        elif family == 2:
            pad = 16 - addrlen
            addr = lambda.vendor.dns.ipv6.inet_ntoa(prefix + b"\x00" * pad)
        else:
...
class EDECode(lambda.vendor.dns.enum.IntEnum):
...
class ReportChannelOption(Option):
    # RFC 9567
    def __init__(self, agent_domain: lambda.vendor.dns.name.Name):

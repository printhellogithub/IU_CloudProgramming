...
from lambda.vendor.dns.dnssectypes import Algorithm, DSDigest, NSEC3Hash
from lambda.vendor.dns.exception import AlgorithmKeyMismatch as AlgorithmKeyMismatch
from lambda.vendor.dns.exception import DeniedByPolicy, UnsupportedAlgorithm, ValidationFailure
from lambda.vendor.dns.rdtypes.ANY.CDNSKEY import CDNSKEY
from lambda.vendor.dns.rdtypes.ANY.CDS import CDS
from lambda.vendor.dns.rdtypes.ANY.DNSKEY import DNSKEY
from lambda.vendor.dns.rdtypes.ANY.DS import DS
from lambda.vendor.dns.rdtypes.ANY.NSEC import NSEC, Bitmap
from lambda.vendor.dns.rdtypes.ANY.NSEC3PARAM import NSEC3PARAM
from lambda.vendor.dns.rdtypes.ANY.RRSIG import RRSIG, sigtime_to_posixtime
from lambda.vendor.dns.rdtypes.dnskeybase import Flag
...
RRsetSigner = Callable[[lambda.vendor.dns.transaction.Transaction, lambda.vendor.dns.rrset.RRset], None]
...
def make_ds(
    ...
    *validating*, a ``bool``.  If ``True``, then policy is checked in
    ...
    if isinstance(name, str):
        name = lambda.vendor.dns.name.from_text(name, origin)
    ...
    ds = dns.rdata.from_wire(
        lambda.vendor.dns.rdataclass.IN, dns.rdatatype.DS, dsrdata, 0, len(dsrdata)
    )
    return cast(DS, ds)
...
def _find_candidate_keys(
    keys: Dict[lambda.vendor.dns.name.Name, dns.rdataset.Rdataset | dns.node.Node], rrsig: RRSIG
) -> List[DNSKEY] | None:
...
def _get_rrname_rdataset(
    rrset: dns.rrset.RRset | Tuple[lambda.vendor.dns.name.Name, dns.rdataset.Rdataset],
) -> Tuple[lambda.vendor.dns.name.Name, dns.rdataset.Rdataset]:
...
def _validate_rrsig(
    rrset: dns.rrset.RRset | Tuple[lambda.vendor.dns.name.Name, dns.rdataset.Rdataset],
    rrsig: RRSIG,
    keys: Dict[lambda.vendor.dns.name.Name, dns.node.Node | dns.rdataset.Rdataset],
    origin: lambda.vendor.dns.name.Name | None = None,
    now: float | None = None,
    policy: Policy | None = None,
) -> None:
...
def _validate(
    rrset: dns.rrset.RRset | Tuple[lambda.vendor.dns.name.Name, dns.rdataset.Rdataset],
    rrsigset: dns.rrset.RRset | Tuple[lambda.vendor.dns.name.Name, dns.rdataset.Rdataset],
    keys: Dict[lambda.vendor.dns.name.Name, dns.node.Node | dns.rdataset.Rdataset],
    origin: lambda.vendor.dns.name.Name | None = None,
    now: float | None = None,
    policy: Policy | None = None,
    ...
    *policy*, a ``dns.dnssec.Policy`` or ``None``.  If ``None``, the default policy,
    ...
    if isinstance(origin, str):
        origin = lambda.vendor.dns.name.from_text(origin, lambda.vendor.dns.name.root)
...
def _sign(
    ...
    signer: lambda.vendor.dns.name.Name,
    dnskey: DNSKEY,
...
def _make_rrsig_signature_data(
    ...
    *origin*, a ``dns.name.Name`` or ``None``, the origin to use for relative
    ...
    if name_len - 1 < rrsig.labels:
        ...
        rrname = lambda.vendor.dns.name.from_text("*", suffix)
...
def nsec3_hash(
    domain: lambda.vendor.dns.name.Name | str,
    salt: str | bytes | None,
    iterations: int,
    algorithm: int | str,
) -> str:
    ...
    *algorithm*, a ``str`` or ``int``, the hash algorithm.
    ...
    if not isinstance(domain, lambda.vendor.dns.name.Name):
        domain = lambda.vendor.dns.name.from_text(domain)
...
def _sign_zone_nsec(
    ...
    def _txn_add_nsec(
        ...
        next_secure: lambda.vendor.dns.name.Name | None,
...
if lambda.vendor.dns._features.have("dnssec"):

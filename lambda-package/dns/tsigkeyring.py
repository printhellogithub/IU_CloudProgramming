...
def from_text(textring: Dict[str, Any]) -> Dict[lambda.vendor.dns.name.Name, Any]:
    ...
    keyring: Dict[lambda.vendor.dns.name.Name, Any] = {}
    for name, value in textring.items():
        kname = lambda.vendor.dns.name.from_text(name)
        if isinstance(value, str):
            keyring[kname] = lambda.vendor.dns.tsig.Key(kname, value).secret
        else:
            (algorithm, secret) = value
            keyring[kname] = lambda.vendor.dns.tsig.Key(kname, secret, algorithm)
    return keyring


def to_text(keyring: Dict[lambda.vendor.dns.name.Name, Any]) -> Dict[str, Any]:

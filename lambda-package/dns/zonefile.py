...
import lambda.vendor.dns.grange
import lambda.vendor.dns.name
import lambda.vendor.dns.node
...
import lambda.vendor.dns.transaction
import lambda.vendor.dns.ttl


class UnknownOrigin(lambda.vendor.dns.exception.DNSException):
    """Unknown origin"""


class CNAMEAndOtherData(lambda.vendor.dns.exception.DNSException):
    """A node has a CNAME and other data"""


def _check_cname_and_other_data(txn, name, rdataset):
    rdataset_kind = lambda.vendor.dns.node.NodeKind.classify_rdataset(rdataset)
    ...
    if (
        node_kind == lambda.vendor.dns.node.NodeKind.CNAME
        and rdataset_kind == lambda.vendor.dns.node.NodeKind.REGULAR
    ):
        raise CNAMEAndOtherData("rdataset type is not compatible with a CNAME node")
    elif (
        node_kind == lambda.vendor.dns.node.NodeKind.REGULAR
        and rdataset_kind == lambda.vendor.dns.node.NodeKind.CNAME
    ):
...
SavedStateType = Tuple[
    lambda.vendor.dns.tokenizer.Tokenizer,
    lambda.vendor.dns.name.Name | None,  # current_origin
    lambda.vendor.dns.name.Name | None,  # last_name
...
class Reader:
    ...
    def __init__(
        self,
        tok: lambda.vendor.dns.tokenizer.Tokenizer,
        rdclass: lambda.vendor.dns.rdataclass.RdataClass,
        txn: dns.transaction.Transaction,
        allow_include: bool = False,
        allow_directives: bool | Iterable[str] = True,
        force_name: lambda.vendor.dns.name.Name | None = None,
    ...
    def _rr_line(self):
        ...
        if self.force_name is not None:
            ...
            name = self.last_name
            if name is None:
                raise lambda.vendor.dns.exception.SyntaxError("the last used name is undefined")
        ...
        if self.force_ttl is not None:
            ...
            ttl = None
            try:
                ttl = lambda.vendor.dns.ttl.from_text(token.value)
                self.last_ttl = ttl
                self.last_ttl_known = True
                token = None
            except lambda.vendor.dns.ttl.BadTTL:
                self.tok.unget(token)
        ...
        if self.force_rdclass is not None:
            ...
            token = self._get_identifier()
            try:
                rdclass = lambda.vendor.dns.rdataclass.from_text(token.value)
            ...
            if rdclass != self.zone_rdclass:
                raise lambda.vendor.dns.exception.SyntaxError("RR class is not zone's class")
        ...
        if self.force_rdtype is not None:
            ...
            token = self._get_identifier()
            try:
                rdtype = lambda.vendor.dns.rdatatype.from_text(token.value)
            except Exception:
                raise lambda.vendor.dns.exception.SyntaxError(f"unknown rdatatype '{token.value}'")
        ...
        try:
            ...
            raise lambda.vendor.dns.exception.SyntaxError(f"caught exception {str(ty)}: {str(va)}")

        if not self.default_ttl_known and rdtype == lambda.vendor.dns.rdatatype.SOA:
            # The pre-RFC2308 and pre-BIND9 behavior inherits the zone default
            # TTL from the SOA minttl if no $TTL statement is present before the
            # SOA is parsed.
            soa_rd = cast(lambda.vendor.dns.rdtypes.ANY.SOA.SOA, rd)
        ...
        if ttl is None:
            raise lambda.vendor.dns.exception.SyntaxError("Missing default TTL value")
    ...
    def _parse_modify(self, side: str) -> Tuple[str, str, int, int, str]:
        ...
        if sign not in ["+", "-"]:
            raise lambda.vendor.dns.exception.SyntaxError(f"invalid offset sign {sign}")
        if base not in ["d", "o", "x", "X", "n", "N"]:
            raise lambda.vendor.dns.exception.SyntaxError(f"invalid type {base}")
    ...
    def _generate_line(self):
        ...
        # Range (required)
        try:
            start, stop, step = lambda.vendor.dns.grange.from_text(token.value)
            token = self.tok.get()
            if not token.is_identifier():
                raise lambda.vendor.dns.exception.SyntaxError
        except Exception:
        ...
        for i in range(start, stop + 1, step):
            ...
            self.last_name = lambda.vendor.dns.name.from_text(
            ...
            while 1:
                ...
                if token.is_eof():
                    ...
                    if c == "$TTL":
                        token = self.tok.get()
                        if not token.is_identifier():
                            raise lambda.vendor.dns.exception.SyntaxError("bad $TTL")
                        self.default_ttl = lambda.vendor.dns.ttl.from_text(token.value)
                        self.default_ttl_known = True
                        self.tok.get_eol()
                    elif c == "$ORIGIN":
                        ...
                        token = self.tok.get()
                        new_origin: lambda.vendor.dns.name.Name | None
                        if token.is_identifier():
                            new_origin = lambda.vendor.dns.name.from_text(
                                token.value, self.current_origin, self.tok.idna_codec
                            )
                            self.tok.get_eol()
                        elif not token.is_eol_or_eof():
                            raise lambda.vendor.dns.exception.SyntaxError("bad origin in $INCLUDE")
                        else:
                        ...
                        self.tok = lambda.vendor.dns.tokenizer.Tokenizer(self.current_file, filename)
                ...
                self.tok.unget(token)
                self._rr_line()
        except lambda.vendor.dns.exception.SyntaxError as detail:
            (filename, line_number) = self.tok.where()
            if detail is None:
                detail = "syntax error"
            ex = lambda.vendor.dns.exception.SyntaxError(f"{filename}:{line_number}: {detail}")
            tb = sys.exc_info()[2]
            raise ex.with_traceback(tb) from None


class RRsetsReaderTransaction(lambda.vendor.dns.transaction.Transaction):
    ...
    def _get_node(self, name):
        ...
        if len(rdatasets) == 0:
            return None
        node = lambda.vendor.dns.node.Node()
    ...
    def _end_transaction(self, commit):
        if commit and self._changed():
            ...
            for (name, _, _), rdataset in self.rdatasets.items():
                rrset = lambda.vendor.dns.rrset.RRset(
...
class RRSetsReaderManager(lambda.vendor.dns.transaction.TransactionManager):
    def __init__(
        self,
        origin: lambda.vendor.dns.name.Name | None = lambda.vendor.dns.name.root,
        ...
        self.rrsets: List[lambda.vendor.dns.rrset.RRset] = []
    ...
    def origin_information(self):
        if self.relativize:
            effective = lambda.vendor.dns.name.empty
        else:
            effective = self.origin
        return (self.origin, self.relativize, effective)

    def set_rrsets(self, rrsets: List[lambda.vendor.dns.rrset.RRset]) -> None:
        self.rrsets = rrsets


def read_rrsets(
    text: Any,
    name: lambda.vendor.dns.name.Name | str | None = None,
    ttl: int | None = None,
    ...
    origin: lambda.vendor.dns.name.Name | str | None = lambda.vendor.dns.name.root,
    relativize: bool = False,
    ...
    *relativize*, a bool.  If ``True``, names are relativized to the *origin*;
    ...
    if isinstance(origin, str):
        origin = lambda.vendor.dns.name.from_text(origin, lambda.vendor.dns.name.root, idna_codec)
    if isinstance(name, str):
        name = lambda.vendor.dns.name.from_text(name, origin, idna_codec)
    if isinstance(ttl, str):
        ttl = lambda.vendor.dns.ttl.from_text(ttl)
    if isinstance(default_ttl, str):
        default_ttl = lambda.vendor.dns.ttl.from_text(default_ttl)
    if rdclass is not None:
        rdclass = lambda.vendor.dns.rdataclass.RdataClass.make(rdclass)
    else:
        rdclass = None
    default_rdclass = lambda.vendor.dns.rdataclass.RdataClass.make(default_rdclass)
    ...
    with manager.writer(True) as txn:
        tok = lambda.vendor.dns.tokenizer.Tokenizer(text, "<input>", idna_codec=idna_codec)
        reader = Reader(
            tok,
            default_rdclass,
            txn,

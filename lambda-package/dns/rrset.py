...
class RRset(lambda.vendor.dns.rdataset.Rdataset):
    ...
    def __repr__(self):
        ...
        if self.deleting is not None:
            dtext = " delete=" + lambda.vendor.dns.rdataclass.to_text(self.deleting)
        else:
            dtext = ""
        return (
            "<DNS "
    ...
    def __eq__(self, other):
        if isinstance(other, RRset):
            if self.name != other.name:
                return False
        elif not isinstance(other, lambda.vendor.dns.rdataset.Rdataset):
            return False
    ...
    def match(self, *args: Any, **kwargs: Any) -> bool:  # type: ignore[override]
        ...
        compatibility.)
        """
        if isinstance(args[0], lambda.vendor.dns.name.Name):
    ...
    def to_wire(  # type: ignore[override]
        self,
        file: Any,
        compress: lambda.vendor.dns.name.CompressType | None = None,  # type: ignore
        origin: lambda.vendor.dns.name.Name | None = None,
        **kw: Dict[str, Any],
    ) -> int:
...
def from_text_list(
    ...
    *relativize_to*, a ``dns.name.Name`` (or ``None``), the origin to use
    ...
    if isinstance(name, str):
        name = lambda.vendor.dns.name.from_text(name, None, idna_codec=idna_codec)
...
def from_rdata(name: lambda.vendor.dns.name.Name | str, ttl: int, *rdatas: Any) -> RRset:
    """Create an RRset with the specified name and TTL, and with

...
import lambda.vendor.dns.btree
...
import lambda.vendor.dns.versioned
...
class Node(lambda.vendor.dns.node.Node):
    __slots__ = ["flags", "id"]
...
class Delegations(lambda.vendor.dns.btree.BTreeSet[dns.name.Name]):
    def get_delegation(self, name: lambda.vendor.dns.name.Name) -> Tuple[lambda.vendor.dns.name.Name | None, bool]:
        ...
        is_subdomain = reln == lambda.vendor.dns.name.NameRelation.SUBDOMAIN
        if is_subdomain or reln == lambda.vendor.dns.name.NameRelation.EQUAL:
            return cut, is_subdomain
        else:
            return None, False

    def is_glue(self, name: lambda.vendor.dns.name.Name) -> bool:
...
class WritableVersion(dns.zone.WritableVersion):
    def __init__(self, zone: dns.zone.Zone, replacement: bool = False):
        ...
        if not replacement:
            ...
            self.nodes: lambda.vendor.dns.btree.BTreeDict[dns.name.Name, Node] = lambda.vendor.dns.btree.BTreeDict(
    ...
    def _is_origin(self, name: lambda.vendor.dns.name.Name) -> bool:
        # Assumes name has already been validated (and thus adjusted to the right
        # relativity too)
        if self.zone.relativize:
            return name == lambda.vendor.dns.name.empty
        else:
    ...
    def update_glue_flag(self, name: lambda.vendor.dns.name.Name, is_glue: bool) -> None:
...
class Bounds:
    name: lambda.vendor.dns.name.Name
    left: lambda.vendor.dns.name.Name
    right: lambda.vendor.dns.name.Name | None
    closest_encloser: lambda.vendor.dns.name.Name
    is_equal: bool
    is_delegation: bool
...
class ImmutableVersion(dns.zone.Version):
    def __init__(self, version: dns.zone.Version):
        ...
        self.nodes = cast(MutableMapping[lambda.vendor.dns.name.Name, dns.node.Node], version.nodes)
        self.nodes.make_immutable()  # type: ignore
        self.delegations = version.delegations
        self.delegations.make_immutable()

    def bounds(self, name: lambda.vendor.dns.name.Name | str) -> Bounds:
        ...
        *name* that is in the zone, either explicitly or by the implied existence
        ...
        if cut is not None:
            ...
            target = name
            is_delegation = False
        c = cast(lambda.vendor.dns.btree.BTreeDict, self.nodes).cursor()
        c.seek(target, False)
        left = c.prev()
        ...
        if right is not None:
            ...
            right_comparison = (
                lambda.vendor.dns.name.NAMERELN_COMMONANCESTOR,
                -1,
                len(origin),
            )
            right_key = None
        ...
        return Bounds(
            ...
            right_key,
            closest_encloser,
            left_comparison[0] == lambda.vendor.dns.name.NameRelation.EQUAL,
            is_delegation,
        )
...
class Zone(dns.versioned.Zone):
    ...
    map_factory: Callable[[], MutableMapping[lambda.vendor.dns.name.Name, dns.node.Node]] = cast(
        Callable[[], MutableMapping[lambda.vendor.dns.name.Name, dns.node.Node]],
        lambda.vendor.dns.btree.BTreeDict[dns.name.Name, Node],
    )

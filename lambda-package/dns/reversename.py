...
ipv4_reverse_domain = lambda.vendor.dns.name.from_text("in-addr.arpa.")
ipv6_reverse_domain = lambda.vendor.dns.name.from_text("ip6.arpa.")


def from_address(
    text: str,
    v4_origin: lambda.vendor.dns.name.Name = ipv4_reverse_domain,
    v6_origin: lambda.vendor.dns.name.Name = ipv6_reverse_domain,
    ...
    *v6_origin*, a ``dns.name.Name`` to append to the labels corresponding to
    ...
    try:
        v6 = lambda.vendor.dns.ipv6.inet_aton(text)
        if lambda.vendor.dns.ipv6.is_mapped(v6):
        ...
        parts = [str(byte) for byte in lambda.vendor.dns.ipv4.inet_aton(text)]
        origin = v4_origin
    return lambda.vendor.dns.name.from_text(".".join(reversed(parts)), origin=origin)
...
def to_address(
    ...
    *v6_origin*, a ``dns.name.Name`` representing the top-level domain for
    ...
    if name.is_subdomain(v4_origin):
        ...
        return lambda.vendor.dns.ipv4.inet_ntoa(lambda.vendor.dns.ipv4.inet_aton(text))
        ...
        text = b":".join(parts)
        # run through inet_ntoa() to check syntax and make pretty.
        return lambda.vendor.dns.ipv6.inet_ntoa(lambda.vendor.dns.ipv6.inet_aton(text))
    else:
        raise lambda.vendor.dns.exception.SyntaxError("unknown reverse-map address family")

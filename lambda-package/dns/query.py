...
import lambda.vendor.dns._tls_util
...
import lambda.vendor.dns.xfr

try:
    import ssl
except ImportError:
    import lambda.vendor.dns._no_ssl as ssl  # type: ignore
...
def _remaining(expiration):
    ...
    if timeout <= 0.0:
        raise lambda.vendor.dns.exception.Timeout
    return timeout
...
_have_httpx = lambda.vendor.dns._features.have("doh")
if _have_httpx:
    ...
    class _NetworkBackend(_CoreNetworkBackend):
        ...
        def connect_tcp(
            ...
            if lambda.vendor.dns.inet.is_address(host):
                ...
                family = self._family
                if local_address:
                    family = lambda.vendor.dns.inet.af_for_address(local_address)
                answers = self._resolver.resolve_name(
                    host, family=family, lifetime=timeout
                )
                addresses = answers.addresses()
            for address in addresses:
                af = lambda.vendor.dns.inet.af_for_address(address)
                if local_address is not None or self._local_port != 0:
                    if local_address is None:
                        local_address = "0.0.0.0"
                    source = lambda.vendor.dns.inet.low_level_address_tuple(
                ...
                try:
                    ...
                    _connect(
                        sock,
                        lambda.vendor.dns.inet.low_level_address_tuple((address, port), af),
                        attempt_expiration,
                    )
        ...
        def __init__(
            ...
            **kwargs,
            ...
            if resolver is None and bootstrap_address is None:
                ...
                import lambda.vendor.dns.resolver
...
class UnexpectedSource(lambda.vendor.dns.exception.DNSException):
    """A DNS query response came from an unexpected address or port."""


class BadResponse(lambda.vendor.dns.exception.FormError):
    """A DNS query response does not respond to the question asked."""


class NoDOH(lambda.vendor.dns.exception.DNSException):
    """DNS over HTTPS (DOH) was requested but the httpx module is not
    available."""


class NoDOQ(lambda.vendor.dns.exception.DNSException):
    """DNS over QUIC (DOQ) was requested but the aioquic module is not
    available."""


# for backwards compatibility
TransferError = lambda.vendor.dns.xfr.TransferError
...
def _addresses_equal(af, a1, a2):
    ...
    try:
        n1 = lambda.vendor.dns.inet.inet_pton(af, a1[0])
        n2 = lambda.vendor.dns.inet.inet_pton(af, a2[0])
...
def _matches_destination(af, from_address, destination, ignore_unexpected):
    ...
    if _addresses_equal(af, from_address, destination) or (
        lambda.vendor.dns.inet.is_multicast(destination[0]) and from_address[1:] == destination[1:]
    ):
        return True
...
def _destination_and_source(
    ...
    destination = None
    try:
        af = lambda.vendor.dns.inet.af_for_address(where)
        destination = where
    except Exception:
    ...
    if source:
        saf = lambda.vendor.dns.inet.af_for_address(source)
        if af:
    ...
    if source_port and not source:
        ...
        try:
            source = lambda.vendor.dns.inet.any_for_af(af)
        except Exception:
    ...
    # tuples.
    if destination:
        destination = lambda.vendor.dns.inet.low_level_address_tuple((destination, port), af)
    if source:
        source = lambda.vendor.dns.inet.low_level_address_tuple((source, source_port), af)
...
def make_ssl_socket(
    ...
    server_hostname: lambda.vendor.dns.name.Name | str | None = None,
    source: Any | None = None,
) -> ssl.SSLSocket:
    """Make a socket.
    ...
    *source* is the source address and port to bind to, if any.  The default is
    ...
    if isinstance(server_hostname, lambda.vendor.dns.name.Name):
...
def https(
    q: lambda.vendor.dns.message.Message,
    where: str,
...
) -> lambda.vendor.dns.message.Message:
    ...
    *http_version*, a ``dns.query.HTTPVersion``, indicating which HTTP version to use.
    ...
    url = where
    if af is not None and lambda.vendor.dns.inet.is_address(where):
    ...
    if bootstrap_address is None:
        ...
        if lambda.vendor.dns.inet.is_address(parsed.hostname):
    ...
    r = lambda.vendor.dns.message.from_wire(
        response.content,
        keyring=q.keyring,
...
def send_udp(
    sock: Any,
    what: lambda.vendor.dns.message.Message | bytes,
    destination: Any,
    ...
    *expiration*, a ``float`` or ``None``, the absolute time at which
    ...
    if isinstance(what, lambda.vendor.dns.message.Message):
        what = what.to_wire()
    sent_time = time.time()
...
def receive_udp(
    ...
    query: lambda.vendor.dns.message.Message | None = None,
) -> Any:
    ...
    *ignore_errors* is ``True``, check that the received message is a response
    ...
    while True:
        ...
        except lambda.vendor.dns.message.Truncated as e:
...
def udp_with_fallback(
    ...
    ignore_errors: bool = False,
) -> Tuple[lambda.vendor.dns.message.Message, bool]:
    ...
    *ignore_errors*, a ``bool``.  If various format errors or response mismatches occur
    ...
    except lambda.vendor.dns.message.Truncated:
        response = tcp(
            q,
            where,
            timeout,
...
def receive_tcp(
    ...
    request_mac: bytes | None = b"",
    ignore_trailing: bool = False,
) -> Tuple[lambda.vendor.dns.message.Message, float]:
...
def make_ssl_context(
    ...
    *aplns* is ``None`` or a list of TLS ALPN (Application Layer Protocol Negotiation)
    ...
    """
    cafile, capath = lambda.vendor.dns._tls_util.convert_verify_to_cafile_and_capath(verify)
...
def _inbound_xfr(
    ...
    query: lambda.vendor.dns.message.Message,
    serial: int | None,
    timeout: float | None,
    ...
    with dns.xfr.Inbound(txn_manager, rdtype, serial, is_udp) as inbound:
        ...
        tsig_ctx = None
        r: lambda.vendor.dns.message.Message | None = None
        while not done:
        ...
        if query.keyring and r is not None and not r.had_tsig:
            raise lambda.vendor.dns.exception.FormError("missing TSIG")
...
def xfr(
    ...
    *keyalgorithm*, a ``dns.name.Name`` or ``str``, the TSIG algorithm to use.
    ...
    q = lambda.vendor.dns.message.make_query(zone, rdtype, rdclass)

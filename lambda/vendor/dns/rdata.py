...
class NoRelativeRdataOrdering(lambda.vendor.dns.exception.DNSException):
...
_constify = lambda.vendor.dns.immutable.constify
...
class Rdata:
    ...
    def covers(self) -> lambda.vendor.dns.rdatatype.RdataType:
        ...
        return lambda.vendor.dns.rdatatype.NONE
    ...
    def _to_wire(
        self,
        file: Any,
        compress: lambda.vendor.dns.name.CompressType | None = None,
        origin: lambda.vendor.dns.name.Name | None = None,
    ...
    def to_generic(self, origin: lambda.vendor.dns.name.Name | None = None) -> "GenericRdata":
    ...
    def to_digestable(self, origin: lambda.vendor.dns.name.Name | None = None) -> bytes:
    ...
    def _cmp(self, other):
        ...
        try:
            ...
            our_relative = False
        except lambda.vendor.dns.name.NeedAbsoluteNameOrOrigin:
            if _allow_relative_comparisons:
                our = self.to_digestable(lambda.vendor.dns.name.root)
            our_relative = True
        try:
            ...
            if _allow_relative_comparisons:
                their = other.to_digestable(lambda.vendor.dns.name.root)
            their_relative = True
        if _allow_relative_comparisons:
    ...
    def __hash__(self):
        return hash(self.to_digestable(lambda.vendor.dns.name.root))
    ...
    def _as_name(cls, value):
        ...
        if isinstance(value, str):
            return lambda.vendor.dns.name.from_text(value)
        elif not isinstance(value, lambda.vendor.dns.name.Name):
    ...
    def _as_ipv4_address(cls, value):
        if isinstance(value, str):
            return lambda.vendor.dns.ipv4.canonicalize(value)
        elif isinstance(value, bytes):
            return lambda.vendor.dns.ipv4.inet_ntoa(value)
        elif isinstance(value, ipaddress.IPv4Address):
            return lambda.vendor.dns.ipv4.inet_ntoa(value.packed)
        else:
    ...
    def _as_ipv6_address(cls, value):
        if isinstance(value, str):
            return lambda.vendor.dns.ipv6.canonicalize(value)
        elif isinstance(value, bytes):
            return lambda.vendor.dns.ipv6.inet_ntoa(value)
        elif isinstance(value, ipaddress.IPv6Address):
            return lambda.vendor.dns.ipv6.inet_ntoa(value.packed)
        else:
...
class GenericRdata(Rdata):
    ...
    def from_text(
        ...
        if not token.is_identifier() or token.value != r"\#":
            raise lambda.vendor.dns.exception.SyntaxError(r"generic rdata does not start with \#")
        length = tok.get_int()
        hex = tok.concatenate_remaining_identifiers(True).encode()
        data = binascii.unhexlify(hex)
        if len(data) != length:
            raise lambda.vendor.dns.exception.SyntaxError("generic rdata hex data has wrong length")
...
def from_text(
    ...
    *idna_codec*, a ``dns.name.IDNACodec``, specifies the IDNA
    ...
    with lambda.vendor.dns.exception.ExceptionWrapper(lambda.vendor.dns.exception.SyntaxError):
        rdata = None
        if cls != GenericRdata:
            # peek at first token
            token = tok.get()
...
def from_wire_parser(
    ...
    *origin*, a ``dns.name.Name`` (or ``None``).  If not ``None``,
    ...
    cls = get_rdata_class(rdclass, rdtype)
    assert cls is not None  # for type checkers
    with lambda.vendor.dns.exception.ExceptionWrapper(lambda.vendor.dns.exception.FormError):
...
class RdatatypeExists(lambda.vendor.dns.exception.DNSException):

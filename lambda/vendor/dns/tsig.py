...
class BadTime(lambda.vendor.dns.exception.DNSException):
    """The current time is not within the TSIG's validity time."""


class BadSignature(lambda.vendor.dns.exception.DNSException):
    """The TSIG signature fails to verify."""


class BadKey(lambda.vendor.dns.exception.DNSException):
    """The TSIG record owner name does not match the key."""


class BadAlgorithm(lambda.vendor.dns.exception.DNSException):
    """The TSIG algorithm does not match the key."""


class PeerError(lambda.vendor.dns.exception.DNSException):
...
HMAC_MD5 = lambda.vendor.dns.name.from_text("HMAC-MD5.SIG-ALG.REG.INT")
HMAC_SHA1 = lambda.vendor.dns.name.from_text("hmac-sha1")
HMAC_SHA224 = lambda.vendor.dns.name.from_text("hmac-sha224")
HMAC_SHA256 = lambda.vendor.dns.name.from_text("hmac-sha256")
HMAC_SHA256_128 = lambda.vendor.dns.name.from_text("hmac-sha256-128")
HMAC_SHA384 = lambda.vendor.dns.name.from_text("hmac-sha384")
HMAC_SHA384_192 = lambda.vendor.dns.name.from_text("hmac-sha384-192")
HMAC_SHA512 = lambda.vendor.dns.name.from_text("hmac-sha512")
HMAC_SHA512_256 = lambda.vendor.dns.name.from_text("hmac-sha512-256")
GSS_TSIG = lambda.vendor.dns.name.from_text("gss-tsig")
    ...
    def parse_tkey_and_step(cls, key, message, keyname):
        ...
        try:
            rrset = message.find_rrset(
                message.answer, keyname, lambda.vendor.dns.rdataclass.ANY, dns.rdatatype.TKEY
            )
            if rrset:
                token = rrset[0].key
...
def _digest(wire, key, rdata, time=None, request_mac=None, ctx=None, multi=None):
    ...
    if first:
        ...
        ctx.update(struct.pack("!H", lambda.vendor.dns.rdataclass.ANY))
...
def validate(
    ...
    if rdata.error != 0:
        if rdata.error == lambda.vendor.dns.rcode.BADSIG:
            raise PeerBadSignature
        elif rdata.error == lambda.vendor.dns.rcode.BADKEY:
            raise PeerBadKey
        elif rdata.error == lambda.vendor.dns.rcode.BADTIME:
            raise PeerBadTime
        elif rdata.error == lambda.vendor.dns.rcode.BADTRUNC:
...
class Key:
    def __init__(
        ...
        secret: bytes | str,
        algorithm: lambda.vendor.dns.name.Name | str = default_algorithm,
    ):
        ...
        if isinstance(algorithm, str):
            algorithm = lambda.vendor.dns.name.from_text(algorithm)

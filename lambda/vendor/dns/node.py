...
_cname_types = {
    lambda.vendor.dns.rdatatype.CNAME,
}

# "neutral" types can coexist with a CNAME and thus are not "other data"
_neutral_types = {
    lambda.vendor.dns.rdatatype.NSEC,  # RFC 4035 section 2.5
    lambda.vendor.dns.rdatatype.NSEC3,  # This is not likely to happen, but not impossible!
    lambda.vendor.dns.rdatatype.KEY,  # RFC 4035 section 2.5, RFC 3007
}


def _matches_type_or_its_signature(rdtypes, rdtype, covers):
    return rdtype in rdtypes or (rdtype == lambda.vendor.dns.rdatatype.RRSIG and covers in rdtypes)
...
class NodeKind(enum.Enum):
    ...
    @classmethod
    def classify(
        cls, rdtype: lambda.vendor.dns.rdatatype.RdataType, covers: lambda.vendor.dns.rdatatype.RdataType
    ) -> "NodeKind":
    ...
    @classmethod
    def classify_rdataset(cls, rdataset: lambda.vendor.dns.rdataset.Rdataset) -> "NodeKind":
...
class Node:
    ...
    def to_text(self, name: lambda.vendor.dns.name.Name, **kw: Dict[str, Any]) -> str:
...
class ImmutableNode(Node):
    ...
    def find_rdataset(
        self,
        rdclass: dns.rdataclass.RdataClass,
        rdtype: lambda.vendor.dns.rdatatype.RdataType,
        covers: lambda.vendor.dns.rdatatype.RdataType = lambda.vendor.dns.rdatatype.NONE,
        create: bool = False,
    ) -> dns.rdataset.Rdataset:
        if create:

...
class NameDict(MutableMapping):
    ...
    def __setitem__(self, key, value):
        if not isinstance(key, lambda.vendor.dns.name.Name):
    ...
    def get_deepest_match(self, name):
        ...
        *name*, a ``dns.name.Name``, the name to find.
        ...
        for i in range(-depth, 0):
            n = lambda.vendor.dns.name.Name(name[i:])
            if n in self:
                return (n, self[n])
        v = self[lambda.vendor.dns.name.empty]
        return (lambda.vendor.dns.name.empty, v)

...
if lambda.vendor.dns._features.have("idna"):
    import lambda.vendor.idna as idna  # type: ignore
...
class NameRelation(lambda.vendor.dns.enum.IntEnum):
...
class EmptyLabel(lambda.vendor.dns.exception.SyntaxError):
    """A DNS label is empty."""


class BadEscape(lambda.vendor.dns.exception.SyntaxError):
    """An escaped code in a text format of DNS name is invalid."""


class BadPointer(lambda.vendor.dns.exception.FormError):
    """A DNS compression pointer points forward instead of backward."""


class BadLabelType(lambda.vendor.dns.exception.FormError):
    """The label type in DNS name wire format is unknown."""


class NeedAbsoluteNameOrOrigin(lambda.vendor.dns.exception.DNSException):
    """An attempt was made to convert a non-absolute name to
    wire when there was also a non-absolute (or missing) origin."""


class NameTooLong(lambda.vendor.dns.exception.FormError):
    """A DNS name is > 255 octets long."""


class LabelTooLong(lambda.vendor.dns.exception.SyntaxError):
    """A DNS label is > 63 octets long."""


class AbsoluteConcatenation(lambda.vendor.dns.exception.DNSException):
    """An attempt was made to append anything other than the
    empty name to an absolute DNS name."""


class NoParent(lambda.vendor.dns.exception.DNSException):
    """An attempt was made to get the parent of the root name
    or the empty name."""


class NoIDNA2008(lambda.vendor.dns.exception.DNSException):
    """IDNA 2008 processing was requested but the idna module is not
    available."""


class IDNAException(lambda.vendor.dns.exception.DNSException):
...
class NeedSubdomainOfOrigin(lambda.vendor.dns.exception.DNSException):
...
def from_wire_parser(parser: "lambda.vendor.dns.wire.Parser") -> Name:
...
def from_wire(message: bytes, current: int) -> Tuple[Name, int]:
    ...
    *current*, an ``int``, is the offset of the beginning of the name
    ...
    parser = lambda.vendor.dns.wire.Parser(message, current)

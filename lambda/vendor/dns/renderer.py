...
class Renderer:
    ...
    def _track_size(self):
        ...
        if self.output.tell() > self.max_size:
            ...
            raise lambda.vendor.dns.exception.TooBig
    ...
    def add_question(self, qname, rdtype, rdclass=lambda.vendor.dns.rdataclass.IN):
    ...
    def add_opt(self, opt, pad=0, opt_size=0, tsig_size=0):
        ...
        if pad:
            ...
            options.append(lambda.vendor.dns.edns.GenericOption(lambda.vendor.dns.edns.OptionType.PADDING, pad))
    ...
    def add_tsig(
        ...
        other_data,
        request_mac,
        algorithm=lambda.vendor.dns.tsig.default_algorithm,
    ):
        """Add a TSIG signature to the message."""

        s = self.output.getvalue()

        if isinstance(secret, lambda.vendor.dns.tsig.Key):
            key = secret
        else:
            key = lambda.vendor.dns.tsig.Key(keyname, secret, algorithm)
        tsig = dns.message.Message._make_tsig(  # pyright: ignore
            keyname, algorithm, 0, fudge, b"", id, tsig_error, other_data
        )
        (tsig, _) = lambda.vendor.dns.tsig.sign(s, key, tsig[0], int(time.time()), request_mac)
    ...
    def add_multi_tsig(
        ...
        (tsig, ctx) = lambda.vendor.dns.tsig.sign(
    ...
    def _write_tsig(self, tsig, keyname):
        ...
        with self._track_size():
            ...
            self.output.write(
                struct.pack("!HHI", lambda.vendor.dns.rdatatype.TSIG, lambda.vendor.dns.rdataclass.ANY, 0)
            )

...
import lambda.vendor.dns._ddr
import lambda.vendor.dns.edns
import lambda.vendor.dns.exception
import lambda.vendor.dns.flags
import lambda.vendor.dns.inet
import lambda.vendor.dns.ipv4
import lambda.vendor.dns.ipv6
import lambda.vendor.dns.message
import lambda.vendor.dns.name
import lambda.vendor.dns.nameserver
import lambda.vendor.dns.query
...
import lambda.vendor.dns.rdtypes.ANY.PTR
import lambda.vendor.dns.rdtypes.svcbbase
import lambda.vendor.dns.reversename
...
class NXDOMAIN(lambda.vendor.dns.exception.DNSException):
...
class YXDOMAIN(lambda.vendor.dns.exception.DNSException):
...
ErrorTuple = Tuple[
    ...
    int,
    Exception | str,
    lambda.vendor.dns.message.Message | None,
]
...
class LifetimeTimeout(lambda.vendor.dns.exception.Timeout):
...
class NoAnswer(lambda.vendor.dns.exception.DNSException):
...
class NoNameservers(lambda.vendor.dns.exception.DNSException):
...
class NotAbsolute(lambda.vendor.dns.exception.DNSException):
    """An absolute domain name is required but a relative name was provided."""


class NoRootSOA(lambda.vendor.dns.exception.DNSException):
    """There is no SOA RR at the DNS root name. This should never happen!"""


class NoMetaqueries(lambda.vendor.dns.exception.DNSException):
    """DNS metaqueries are not allowed."""


class NoResolverConfiguration(lambda.vendor.dns.exception.DNSException):
...
class Answer:
    ...
    def __init__(
        self,
        qname: lambda.vendor.dns.name.Name,
        rdtype: dns.rdatatype.RdataType,
        rdclass: dns.rdataclass.RdataClass,
        response: lambda.vendor.dns.message.QueryMessage,
...
class EmptyHostAnswers(lambda.vendor.dns.exception.DNSException):
...
class HostAnswers(Answers):
    ...
    def make(
        ...
        if v6 is not None and (add_empty or v6.rrset):
            answers[lambda.vendor.dns.rdatatype.AAAA] = v6
        if v4 is not None and (add_empty or v4.rrset):
            answers[lambda.vendor.dns.rdatatype.A] = v4
        return answers
    ...
    def addresses_and_families(
        ...
        if family == socket.AF_UNSPEC:
            ...
            answer = self.get(lambda.vendor.dns.rdatatype.AAAA)
        elif family == socket.AF_INET:
            answer = self.get(lambda.vendor.dns.rdatatype.A)
    ...
    def canonical_name(self) -> lambda.vendor.dns.name.Name:
        answer = self.get(lambda.vendor.dns.rdatatype.AAAA, self.get(lambda.vendor.dns.rdatatype.A))
        if answer is None:
...
CacheKey = Tuple[lambda.vendor.dns.name.Name, dns.rdatatype.RdataType, dns.rdataclass.RdataClass]
...
class _Resolution:
    ...
    def __init__(
        ...
        if isinstance(qname, str):
            qname = lambda.vendor.dns.name.from_text(qname, None)
        rdtype = lambda.vendor.dns.rdatatype.RdataType.make(rdtype)
        if lambda.vendor.dns.rdatatype.is_metatype(rdtype):
            raise NoMetaqueries
        rdclass = lambda.vendor.dns.rdataclass.RdataClass.make(rdclass)
        if lambda.vendor.dns.rdataclass.is_metaclass(rdclass):
            raise NoMetaqueries
        ...
        self.nxdomain_responses: Dict[lambda.vendor.dns.name.Name, lambda.vendor.dns.message.QueryMessage] = {}
        # Initialize other things to help analysis tools
        self.qname = lambda.vendor.dns.name.empty
        self.nameservers: List[lambda.vendor.dns.nameserver.Nameserver] = []
        self.current_nameservers: List[lambda.vendor.dns.nameserver.Nameserver] = []
        self.errors: List[ErrorTuple] = []
        self.nameserver: lambda.vendor.dns.nameserver.Nameserver | None = None
        self.tcp_attempt = False
        self.retry_with_tcp = False
        self.request: lambda.vendor.dns.message.QueryMessage | None = None
        self.backoff = 0.0

    def next_request(
        self,
    ) -> Tuple[lambda.vendor.dns.message.QueryMessage | None, Answer | None]:
        ...
        while len(self.qnames) > 0:
            ...
            if self.resolver.cache:
                ...
                if answer is not None:
                    if answer.rrset is None and self.raise_on_no_answer:
                        ...
                        return (None, answer)
                answer = self.resolver.cache.get(
                    (self.qname, lambda.vendor.dns.rdatatype.ANY, self.rdclass)
                )
                if answer is not None and answer.response.rcode() == lambda.vendor.dns.rcode.NXDOMAIN:
            ...
            # Build the request
            request = lambda.vendor.dns.message.make_query(self.qname, self.rdtype, self.rdclass)
    ...
    def next_nameserver(self) -> Tuple[lambda.vendor.dns.nameserver.Nameserver, bool, float]:
    ...
    def query_result(
        self, response: lambda.vendor.dns.message.Message | None, ex: Exception | None
        ...
        if ex:
            ...
            if (
                isinstance(ex, lambda.vendor.dns.exception.FormError)
            ...
            elif isinstance(ex, lambda.vendor.dns.message.Truncated):
                if self.tcp_attempt:
        ...
        assert isinstance(response, lambda.vendor.dns.message.QueryMessage)
        rcode = response.rcode()
        if rcode == lambda.vendor.dns.rcode.NOERROR:
            try:
                answer = Answer(
                    self.qname,
                    self.rdtype,
            ...
            if answer.rrset is None and self.raise_on_no_answer:
                raise NoAnswer(response=answer.response)
            return (answer, True)
        elif rcode == lambda.vendor.dns.rcode.NXDOMAIN:
            # Further validate the response by making an Answer, even
            # if we aren't going to cache it.
            try:
                answer = Answer(
                    self.qname, lambda.vendor.dns.rdatatype.ANY, lambda.vendor.dns.rdataclass.IN, response
                )
            except Exception as e:
                self.errors.append(
                    (
            ...
            if self.resolver.cache:
                self.resolver.cache.put(
                    (self.qname, lambda.vendor.dns.rdatatype.ANY, self.rdclass), answer
                )
            # Make next_nameserver() return None, so caller breaks its
            # inner loop and calls next_request().
            return (None, True)
        elif rcode == lambda.vendor.dns.rcode.YXDOMAIN:
            yex = YXDOMAIN()
            self.errors.append(
                (
                    str(self.nameserver),
            ...
            # rcode in it.
            #
            if rcode != lambda.vendor.dns.rcode.SERVFAIL or not self.resolver.retry_servfail:
                self.nameservers.remove(self.nameserver)
            self.errors.append(
                (
                    str(self.nameserver),
                    self.tcp_attempt,
                    self.nameserver.answer_port(),
                    lambda.vendor.dns.rcode.to_text(rcode),
                    response,
                )
            )
            return (None, False)
...
class BaseResolver:
    ...
    edns: int
    ednsflags: int
    ednsoptions: List[lambda.vendor.dns.edns.Option] | None
    payload: int
    cache: Any
    flags: int | None
    retry_servfail: bool
    ...
    def use_edns(
        ...
        ednsflags: int = 0,
        payload: int = lambda.vendor.dns.message.DEFAULT_EDNS_PAYLOAD,
    ...
    def _enrich_nameservers(
        ...
        if isinstance(nameservers, list | tuple):
            for nameserver in nameservers:
                ...
                elif lambda.vendor.dns.inet.is_address(nameserver):
    ...
    def try_ddr(self, lifetime: float = 5.0) -> None:
        ...
        *lifetime*, a float, is the maximum time to spend attempting DDR.  The default
        ...
        try:
            ...
            answer = self.resolve(
                lambda.vendor.dns._ddr._local_resolver_name, "SVCB", lifetime=lifetime
            )
            timeout = dns.query._remaining(expiration)
            nameservers = lambda.vendor.dns._ddr._get_nameservers_sync(answer, timeout)
            if len(nameservers) > 0:
                self.nameservers = nameservers
        except Exception:  # pragma: no cover
            pass
...
def make_resolver_at(
    ...
    *resolver*, a ``dns.resolver.Resolver`` or ``None``, the resolver to use for
    ...
    if isinstance(where, str) and lambda.vendor.dns.inet.is_address(where):
...
def _getaddrinfo(
    ...
    try:
        ...
        # IPv6 address.
        lambda.vendor.dns.inet.af_for_address(host)
        return _original_getaddrinfo(host, service, family, socktype, proto, flags)
    except Exception:
        pass
    ...
    for addr, af in addrs:
        for socktype in socktypes:
            for sockproto in _protocols_for_socktype[socktype]:
                proto = int(sockproto)
                addr_tuple = lambda.vendor.dns.inet.low_level_address_tuple((addr, port), af)
...
def _gethostbyaddr(ip):
    try:
        lambda.vendor.dns.ipv6.inet_aton(ip)
        sockaddr = (ip, 80, 0, 0)
        family = socket.AF_INET6
    except Exception:
        try:
            lambda.vendor.dns.ipv4.inet_aton(ip)
        except Exception:
            raise socket.gaierror(socket.EAI_NONAME, "Name or service not known")
        sockaddr = (ip, 80)
        family = socket.AF_INET
    ...
    bin_ip = lambda.vendor.dns.inet.inet_pton(family, ip)
    for item in tuples:
        addr = item[4][0]
        assert isinstance(addr, str)
        bin_addr = lambda.vendor.dns.inet.inet_pton(family, addr)
        if bin_ip == bin_addr:

...
_local_resolver_name = lambda.vendor.dns.name.from_text("_dns.resolver.arpa")
...
class _SVCBInfo:
    ...
    def make_tls_context(self):
        ssl = lambda.vendor.dns.query.ssl
    ...
    def ddr_tls_check_sync(self, lifetime):
        ...
        with socket.create_connection(
            ...
            with ctx.wrap_socket(s, server_hostname=self.hostname) as ts:
                ts.settimeout(lambda.vendor.dns.query._remaining(expiration))
                ts.do_handshake()
                cert = ts.getpeercert()
    ...
    async def ddr_tls_check_async(self, lifetime, backend=None):
        ...
        async with await backend.make_socket(
            lambda.vendor.dns.inet.af_for_address(self.bootstrap_address),
            socket.SOCK_STREAM,
            0,
            None,
            ...
            cert = await ts.getpeercert(lambda.vendor.dns.query._remaining(expiration))
            return self.ddr_check_certificate(cert)


def _extract_nameservers_from_svcb(answer):
    bootstrap_address = answer.nameserver
    if not lambda.vendor.dns.inet.is_address(bootstrap_address):
        return []
    infos = []
    for rr in answer.rrset.processing_order():
        nameservers = []
        param = rr.params.get(lambda.vendor.dns.rdtypes.svcbbase.ParamKey.ALPN)
        if param is None:
            continue
        alpns = set(param.ids)
        host = rr.target.to_text(omit_final_dot=True)
        port = None
        param = rr.params.get(lambda.vendor.dns.rdtypes.svcbbase.ParamKey.PORT)
        if param is not None:
            port = param.port
        # For now we ignore address hints and address resolution and always use the
        # bootstrap address
        if b"h2" in alpns:
            param = rr.params.get(lambda.vendor.dns.rdtypes.svcbbase.ParamKey.DOHPATH)

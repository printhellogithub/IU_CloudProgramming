...
import lambda.vendor.dns.rdtypes
import lambda.vendor.dns.rdtypes.ANY
import lambda.vendor.dns.rdtypes.ANY.SMIMEA
import lambda.vendor.dns.rdtypes.ANY.SOA
import lambda.vendor.dns.rdtypes.svcbbase
import lambda.vendor.dns.serial
import lambda.vendor.dns.transaction
import lambda.vendor.dns.tsig
import lambda.vendor.dns.zone


class TransferError(lambda.vendor.dns.exception.DNSException):
    """A zone transfer response got a non-zero rcode."""

    def __init__(self, rcode):
        message = f"Zone transfer error: {lambda.vendor.dns.rcode.to_text(rcode)}"
        super().__init__(message)
        self.rcode = rcode


class SerialWentBackwards(lambda.vendor.dns.exception.FormError):
    """The current serial number is less than the serial we know."""


class UseTCP(lambda.vendor.dns.exception.DNSException):
...
class Inbound:
    ...
    def __init__(
        self,
        txn_manager: lambda.vendor.dns.transaction.TransactionManager,
        rdtype: lambda.vendor.dns.rdatatype.RdataType = lambda.vendor.dns.rdatatype.AXFR,
        ...
        *is_udp*, a ``bool`` indidicates if UDP is being used for this
        ...
        self.txn: lambda.vendor.dns.transaction.Transaction | None = None
        self.rdtype = rdtype
        if rdtype == lambda.vendor.dns.rdatatype.IXFR:
            if serial is None:
                raise ValueError("a starting serial must be supplied for IXFRs")
            self.incremental = True
        elif rdtype == lambda.vendor.dns.rdatatype.AXFR:
            if is_udp:
        ...
        self.soa_rdataset: lambda.vendor.dns.rdataset.Rdataset | None = None
        self.done = False
        self.expecting_SOA = False
        self.delete_mode = False

    def process_message(self, message: lambda.vendor.dns.message.Message) -> bool:
        ...
        if len(message.question) > 0:
            if message.question[0].name != self.origin:
                raise lambda.vendor.dns.exception.FormError("wrong question name")
            if message.question[0].rdtype != self.rdtype:
                raise lambda.vendor.dns.exception.FormError("wrong question rdatatype")
        answer_index = 0
        if self.soa_rdataset is None:
            ...
            if not message.answer or message.answer[0].name != self.origin:
                raise lambda.vendor.dns.exception.FormError("No answer or RRset not for zone origin")
            rrset = message.answer[0]
            rdataset = rrset
            if rdataset.rdtype != lambda.vendor.dns.rdatatype.SOA:
                raise lambda.vendor.dns.exception.FormError("first RRset is not an SOA")
            answer_index = 1
            self.soa_rdataset = rdataset.copy()  # pyright: ignore
            if self.incremental:
                assert self.soa_rdataset is not None
                soa = cast(lambda.vendor.dns.rdtypes.ANY.SOA.SOA, self.soa_rdataset[0])
                if soa.serial == self.serial:
                    #
                    # We're already up-to-date.
                    #
                    self.done = True
                elif lambda.vendor.dns.serial.Serial(soa.serial) < self.serial:
                    # It went backwards!
        ...
        for rrset in message.answer[answer_index:]:
            ...
            rdataset = rrset
            if self.done:
                raise lambda.vendor.dns.exception.FormError("answers after final SOA")
            assert self.txn is not None  # for mypy
            if rdataset.rdtype == lambda.vendor.dns.rdatatype.SOA and name == self.origin:
                #
                ...
                if rdataset == self.soa_rdataset and (
                    ...
                    #
                    soa = cast(lambda.vendor.dns.rdtypes.ANY.SOA.SOA, rdataset[0])
                    if self.expecting_SOA:
                        # We got an empty IXFR sequence!
                        raise lambda.vendor.dns.exception.FormError("empty IXFR sequence")
                    if self.incremental and self.serial != soa.serial:
                        raise lambda.vendor.dns.exception.FormError("unexpected end of IXFR sequence")
                    ...
                    if self.incremental:
                        ...
                        raise lambda.vendor.dns.exception.FormError("unexpected origin SOA in AXFR")
                continue
            if self.expecting_SOA:
                #
        ...
        if self.is_udp and not self.done:
            ...
            #
            raise lambda.vendor.dns.exception.FormError("unexpected end of UDP IXFR")
        return self.done
...
def make_query(
    ...
    *txn_manager*; it is the caller's responsibility to ensure there
    ...
    q = lambda.vendor.dns.message.make_query(
        zone_origin,
        rdtype,
        rdclass,
        use_edns,

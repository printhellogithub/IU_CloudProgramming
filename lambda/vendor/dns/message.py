...
import lambda.vendor.dns.edns
import lambda.vendor.dns.entropy
import lambda.vendor.dns.enum
import lambda.vendor.dns.exception
import lambda.vendor.dns.flags
import lambda.vendor.dns.name
import lambda.vendor.dns.opcode
import lambda.vendor.dns.rcode
import lambda.vendor.dns.rdata
import lambda.vendor.dns.rdataclass
import lambda.vendor.dns.rdatatype
import lambda.vendor.dns.rdtypes.ANY.OPT
import lambda.vendor.dns.rdtypes.ANY.SOA
import lambda.vendor.dns.rdtypes.ANY.TSIG
import lambda.vendor.dns.renderer
import lambda.vendor.dns.rrset
import lambda.vendor.dns.tokenizer
import lambda.vendor.dns.tsig
import lambda.vendor.dns.ttl
import lambda.vendor.dns.wire


class ShortHeader(lambda.vendor.dns.exception.FormError):
    """The DNS packet passed to from_wire() is too short."""


class TrailingJunk(lambda.vendor.dns.exception.FormError):
    """The DNS packet passed to from_wire() has extra junk at the end of it."""


class UnknownHeaderField(lambda.vendor.dns.exception.DNSException):
    """The header field name was not recognized when converting from text
    into a message."""


class BadEDNS(lambda.vendor.dns.exception.FormError):
    """An OPT record occurred somewhere other than
    the additional data section."""


class BadTSIG(lambda.vendor.dns.exception.FormError):
    """A TSIG record occurred somewhere other than the end of
    the additional data section."""


class UnknownTSIGKey(lambda.vendor.dns.exception.DNSException):
    """A TSIG with an unknown key was received."""


class Truncated(lambda.vendor.dns.exception.DNSException):
...
class NotQueryResponse(lambda.vendor.dns.exception.DNSException):
    """Message is not a response to a query."""


class ChainTooLong(lambda.vendor.dns.exception.DNSException):
    """The CNAME chain is too long."""


class AnswerForNXDOMAIN(lambda.vendor.dns.exception.DNSException):
    """The rcode is NXDOMAIN but an answer was found."""


class NoPreviousName(lambda.vendor.dns.exception.SyntaxError):
    """No previous name was known."""


class MessageSection(lambda.vendor.dns.enum.IntEnum):
    """Message sections"""
...
IndexKeyType = Tuple[
    int,
    lambda.vendor.dns.name.Name,
    dns.rdataclass.RdataClass,
    dns.rdatatype.RdataType,
    dns.rdatatype.RdataType | None,
    dns.rdataclass.RdataClass | None,
]
IndexType = Dict[IndexKeyType, lambda.vendor.dns.rrset.RRset]
SectionType = int | str | List[lambda.vendor.dns.rrset.RRset]
...
class Message:
    ...
    def __init__(self, id: int | None = None):
        if id is None:
            self.id = lambda.vendor.dns.entropy.random_16()
        else:
            self.id = id
        self.flags = 0
        self.sections: List[List[lambda.vendor.dns.rrset.RRset]] = [[], [], [], []]
        self.opt: lambda.vendor.dns.rrset.RRset | None = None
        self.request_payload = 0
        self.pad = 0
        self.keyring: Any = None
        self.tsig: lambda.vendor.dns.rrset.RRset | None = None
        self.want_tsig_sign = False
        self.request_mac = b""
        self.xfr = False
        self.origin: lambda.vendor.dns.name.Name | None = None
    ...
    @property
    def question(self) -> List[lambda.vendor.dns.rrset.RRset]:
    ...
    @property
    def answer(self) -> List[lambda.vendor.dns.rrset.RRset]:
        """The answer section."""
    ...
    @property
    def authority(self) -> List[lambda.vendor.dns.rrset.RRset]:
    ...
    @property
    def additional(self) -> List[lambda.vendor.dns.rrset.RRset]:
        """The additional data section."""
    ...
    def to_text(
        self,
        origin: lambda.vendor.dns.name.Name | None = None,
        ...
        **kw: Dict[str, Any],
        ...
        s.write(f"flags {lambda.vendor.dns.flags.to_text(self.flags)}\n")
        if self.edns >= 0:
            s.write(f"edns {self.edns}\n")
            if self.ednsflags != 0:
                s.write(f"eflags {lambda.vendor.dns.flags.edns_to_text(self.ednsflags)}\n")
    ...
    def is_response(self, other: "Message") -> bool:
        ...
        if (
            other.flags & lambda.vendor.dns.flags.QR == 0
            or self.id != other.id
    ...
    def find_rrset(
        self,
        section: SectionType,
        name: lambda.vendor.dns.name.Name,
        ...
        idna_codec: lambda.vendor.dns.name.IDNACodec | None = None,
    ) -> dns.rrset.RRset:
        ...
        *idna_codec*, a ``dns.name.IDNACodec``, specifies the IDNA
        ...
        if isinstance(name, str):
            name = lambda.vendor.dns.name.from_text(name, idna_codec=idna_codec)
    ...
    def to_wire(
        ...
        *prefer_truncation*, a ``bool``, should be set to ``True`` if the caller
        ...
        except lambda.vendor.dns.exception.TooBig:
            if prefer_truncation:
                if r.section < dns.renderer.ADDITIONAL:
                    r.flags |= lambda.vendor.dns.flags.TC
            else:
                raise
        r.release_reserved()
        if self.opt is not None:
    ...
    def use_tsig(
        self,
        keyring: Any,
        keyname: lambda.vendor.dns.name.Name | str | None = None,
        fudge: int = 300,
        original_id: int | None = None,
        tsig_error: int = 0,
        other_data: bytes = b"",
        algorithm: lambda.vendor.dns.name.Name | str = dns.tsig.default_algorithm,
    ) -> None:
        ...
        *algorithm*, a ``dns.name.Name`` or ``str``, the TSIG algorithm to use.  This is
        ...
        if isinstance(keyring, dns.tsig.Key):
            ...
            if isinstance(keyname, str):
                keyname = lambda.vendor.dns.name.from_text(keyname)
            if keyname is None:
    ...
    @property
    def keyname(self) -> lambda.vendor.dns.name.Name | None:
        if self.tsig:
    ...
    @property
    def keyalgorithm(self) -> lambda.vendor.dns.name.Name | None:
        if self.tsig:
    ...
    def _make_opt(flags=0, payload=DEFAULT_EDNS_PAYLOAD, options=None):
        ...
        return dns.rrset.from_rdata(lambda.vendor.dns.name.root, int(flags), opt)
    ...
    def use_edns(
        ...
        options: List[lambda.vendor.dns.edns.Option] | None = None,
        pad: int = 0,
    ) -> None:
    ...
    def want_dnssec(self, wanted: bool = True) -> None:
        ...
        *wanted*, a ``bool``.  If ``True``, then DNSSEC data is
        ...
        if wanted:
            self.ednsflags |= lambda.vendor.dns.flags.DO
        elif self.opt:
            self.ednsflags &= ~int(lambda.vendor.dns.flags.DO)
    ...
    def get_options(self, otype: lambda.vendor.dns.edns.OptionType) -> List[lambda.vendor.dns.edns.Option]:
        """Return the list of options of the specified type."""
        return [option for option in self.options if option.otype == otype]

    def extended_errors(self) -> List[lambda.vendor.dns.edns.EDEOption]:
        """Return the list of Extended DNS Error (EDE) options in the message"""
        return cast(List[lambda.vendor.dns.edns.EDEOption], self.get_options(lambda.vendor.dns.edns.OptionType.EDE))
    ...
    def _parse_special_rr_header(self, section, count, position, name, rdclass, rdtype):
        if rdtype == dns.rdatatype.OPT:
            if (
                ...
                or self.opt
                or name != lambda.vendor.dns.name.root
            ):
                raise BadEDNS
...
class ChainingResult:
    ...
    def __init__(
        self,
        canonical_name: lambda.vendor.dns.name.Name,
...
class QueryMessage(Message):
    def resolve_chaining(self) -> ChainingResult:
        ...
        """
        if self.flags & lambda.vendor.dns.flags.QR == 0:
            raise NotQueryResponse
        if len(self.question) != 1:
            raise lambda.vendor.dns.exception.FormError
        ...
        if answer is None:
            ...
            while True:
                ...
                try:
                    ...
                    except lambda.vendor.dns.name.NoParent:
                        break
        return ChainingResult(qname, answer, min_ttl, cnames)

    def canonical_name(self) -> lambda.vendor.dns.name.Name:
...
class _WireReader:
    ...
    def read(self):
        ...
        self.message.flags = lambda.vendor.dns.flags.Flag(flags)
...
def from_wire(
    ...
    *continue_on_error*, a ``bool``.  If ``True``, try to continue parsing even if
    ...
    try:
        m = reader.read()
    except lambda.vendor.dns.exception.FormError:
        if (
            reader.message
            and (reader.message.flags & lambda.vendor.dns.flags.TC)
    ...
    if m.flags & lambda.vendor.dns.flags.TC and raise_on_truncation:
...
class _TextReader:
    ...
    def __init__(
        self,
        text: str,
        idna_codec: lambda.vendor.dns.name.IDNACodec | None,
        one_rr_per_rrset: bool = False,
        origin: lambda.vendor.dns.name.Name | None = None,
        relativize: bool = True,
        relativize_to: lambda.vendor.dns.name.Name | None = None,
    ):
        ...
        if what == "id":
            ...
            while True:
                ...
                if not token.is_identifier():
                    ...
                    break
                self.flags = self.flags | lambda.vendor.dns.flags.from_text(token.value)
        elif what == "edns":
            ...
            while True:
                ...
                if not token.is_identifier():
                    self.tok.unget(token)
                    break
                self.ednsflags = self.ednsflags | lambda.vendor.dns.flags.edns_from_text(token.value)
        elif what == "payload":
    ...
    def _question_line(self, section_number):
        ...
        token = self.tok.get()
        if not token.is_identifier():
            raise lambda.vendor.dns.exception.SyntaxError
        # Class
        try:
            ...
            if not token.is_identifier():
                raise lambda.vendor.dns.exception.SyntaxError
        except lambda.vendor.dns.exception.SyntaxError:
            raise lambda.vendor.dns.exception.SyntaxError
        except Exception:
    ...
    def _rr_line(self, section_number):
        ...
        if not empty and token.is_eol_or_eof():
            raise lambda.vendor.dns.exception.UnexpectedEnd
...
def make_query(
    qname: lambda.vendor.dns.name.Name | str,
    ...
    *pad*, a non-negative ``int``.  If 0, the default, do not pad; otherwise add
    ...
    if isinstance(qname, str):
        qname = lambda.vendor.dns.name.from_text(qname, idna_codec=idna_codec)
    rdtype = dns.rdatatype.RdataType.make(rdtype)
    rdclass = dns.rdataclass.RdataClass.make(rdclass)
    m = QueryMessage(id=id)
    m.flags = lambda.vendor.dns.flags.Flag(flags)
    ...
    if query.edns >= 0:
        if pad is None:
            ...
            for option in query.options:
                if option.otype == lambda.vendor.dns.edns.OptionType.PADDING:
                    pad = 468

...
def _exponent_of(what, desc):
    ...
    if exp is None or exp < 0:
        raise lambda.vendor.dns.exception.SyntaxError(f"{desc} value out of bounds")
    return exp
...
def _decode_size(what, desc):
    exponent = what & 0x0F
    if exponent > 9:
        raise lambda.vendor.dns.exception.FormError(f"bad {desc} exponent")
    base = (what & 0xF0) >> 4
    if base > 9:
        raise lambda.vendor.dns.exception.FormError(f"bad {desc} base")
...
class LOC(lambda.vendor.dns.rdata.Rdata):
    """LOC record"""
    ...
    def from_text(
        ...
        if t.isdigit():
            ...
            if "." in t:
                ...
                if not seconds.isdigit():
                    raise lambda.vendor.dns.exception.SyntaxError("bad latitude seconds value")
                latitude[2] = int(seconds)
                l = len(milliseconds)
                if l == 0 or l > 3 or not milliseconds.isdigit():
                    raise lambda.vendor.dns.exception.SyntaxError("bad latitude milliseconds value")
                if l == 1:
                    m = 100
                elif l == 2:
                    m = 10
        ...
        if t == "S":
            ...
            raise lambda.vendor.dns.exception.SyntaxError("bad latitude hemisphere value")
        ...
        if t.isdigit():
            ...
            if "." in t:
                (seconds, milliseconds) = t.split(".")
                if not seconds.isdigit():
                    raise lambda.vendor.dns.exception.SyntaxError("bad longitude seconds value")
                longitude[2] = int(seconds)
                l = len(milliseconds)
                if l == 0 or l > 3 or not milliseconds.isdigit():
                    raise lambda.vendor.dns.exception.SyntaxError("bad longitude milliseconds value")
                if l == 1:
                    m = 100
                elif l == 2:
                    m = 10
        ...
        if t == "W":
            ...
            raise lambda.vendor.dns.exception.SyntaxError("bad longitude hemisphere value")
    ...
    def from_wire_parser(cls, rdclass, rdtype, parser, origin=None):
        ...
        if version != 0:
            raise lambda.vendor.dns.exception.FormError("LOC version not zero")
        if latitude < _MIN_LATITUDE or latitude > _MAX_LATITUDE:
            raise lambda.vendor.dns.exception.FormError("bad latitude")
        ...
        if longitude < _MIN_LONGITUDE or longitude > _MAX_LONGITUDE:
            raise lambda.vendor.dns.exception.FormError("bad longitude")

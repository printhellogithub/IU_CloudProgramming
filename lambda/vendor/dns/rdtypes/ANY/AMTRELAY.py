...
class Relay(lambda.vendor.dns.rdtypes.util.Gateway):
    name = "AMTRELAY relay"
...
class AMTRELAY(lambda.vendor.dns.rdata.Rdata):
    """AMTRELAY record"""
    ...
    def from_text(
        ...
        if discovery_optional > 1:
            raise lambda.vendor.dns.exception.SyntaxError("expecting 0 or 1")
        discovery_optional = bool(discovery_optional)
        relay_type = tok.get_uint8()
        if relay_type > 0x7F:
            raise lambda.vendor.dns.exception.SyntaxError("expecting an integer <= 127")

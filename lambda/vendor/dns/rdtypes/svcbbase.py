...
class UnknownParamKey(lambda.vendor.dns.exception.DNSException):
    """Unknown SVCB ParamKey"""


class ParamKey(lambda.vendor.dns.enum.IntEnum):
    """SVCB ParamKey"""
...
class GenericParam(Param):
    ...
    def __init__(self, value):
        self.value = lambda.vendor.dns.rdata.Rdata._as_bytes(value, True)
    ...
    def to_text(self):
        return '"' + lambda.vendor.dns.rdata._escapify(self.value) + '"'
...
class MandatoryParam(Param):
    ...
    def from_wire_parser(cls, parser, origin=None):  # pylint: disable=W0613
        ...
        while parser.remaining() > 0:
            ...
            if key < last_key:
                raise lambda.vendor.dns.exception.FormError("manadatory keys not ascending")
            last_key = key
            keys.append(key)
        return cls(keys)
...
class ALPNParam(Param):
    def __init__(self, ids):
        self.ids = lambda.vendor.dns.rdata.Rdata._as_tuple(
            ids, lambda x: lambda.vendor.dns.rdata.Rdata._as_bytes(x, True, 255, False)
        )
    ...
    def to_text(self):
        ...
        return '"' + lambda.vendor.dns.rdata._escapify(value.encode()) + '"'
...
class PortParam(Param):
    def __init__(self, port):
        self.port = lambda.vendor.dns.rdata.Rdata._as_uint16(port)
...
class IPv4HintParam(Param):
    def __init__(self, addresses):
        self.addresses = lambda.vendor.dns.rdata.Rdata._as_tuple(
            addresses, lambda.vendor.dns.rdata.Rdata._as_ipv4_address
        )
    ...
    def from_wire_parser(cls, parser, origin=None):  # pylint: disable=W0613
        ...
        while parser.remaining() > 0:
            ...
            addresses.append(lambda.vendor.dns.ipv4.inet_ntoa(ip))
        return cls(addresses)

    def to_wire(self, file, origin=None):  # pylint: disable=W0613
        for address in self.addresses:
            file.write(lambda.vendor.dns.ipv4.inet_aton(address))
...
class IPv6HintParam(Param):
    def __init__(self, addresses):
        self.addresses = lambda.vendor.dns.rdata.Rdata._as_tuple(
            addresses, lambda.vendor.dns.rdata.Rdata._as_ipv6_address
        )
    ...
    def from_wire_parser(cls, parser, origin=None):  # pylint: disable=W0613
        ...
        while parser.remaining() > 0:
            ...
            addresses.append(lambda.vendor.dns.ipv6.inet_ntoa(ip))
        return cls(addresses)

    def to_wire(self, file, origin=None):  # pylint: disable=W0613
        for address in self.addresses:
            file.write(lambda.vendor.dns.ipv6.inet_aton(address))


@dns.immutable.immutable
class ECHParam(Param):
    def __init__(self, ech):
        self.ech = lambda.vendor.dns.rdata.Rdata._as_bytes(ech, True)
...
def _validate_and_define(params, key, value):
    ...
    if value is None:
        ...
        if force_generic:
            value = cls.from_wire_parser(lambda.vendor.dns.wire.Parser(_unescape(value)))
        else:
            value = cls.from_value(value)
    params[key] = value
...
@dns.immutable.immutable
class SVCBBase(lambda.vendor.dns.rdata.Rdata):
    ...
    def __init__(self, rdclass, rdtype, priority, target, params):
        ...
        self.params = lambda.vendor.dns.immutable.Dict(params)
    ...
    def from_text(
        ...
        while True:
            ...
            if token.is_eol_or_eof():
                tok.unget(token)
                break
            if token.ttype != lambda.vendor.dns.tokenizer.IDENTIFIER:
            ...
            if equals == len(token.value) - 1:
                ...
                if token.ttype != lambda.vendor.dns.tokenizer.QUOTED_STRING:
    ...
    def _to_wire(self, file, compress=None, origin=None, canonicalize=False):
        ...
        for key in sorted(self.params):
            file.write(struct.pack("!H", key))
            value = self.params[key]
            with lambda.vendor.dns.renderer.prefixed_length(file, 2):
    ...
    def from_wire_parser(cls, rdclass, rdtype, parser, origin=None):
        ...
        if priority == 0 and parser.remaining() != 0:
            raise lambda.vendor.dns.exception.FormError("parameters in AliasMode")
        params = {}
        prior_key = -1
        while parser.remaining() > 0:
            key = parser.get_uint16()
            if key < prior_key:
                raise lambda.vendor.dns.exception.FormError("keys not in order")
            prior_key = key

...
class Nameserver:
    ...
    def query(
        self,
        request: lambda.vendor.dns.message.QueryMessage,
        timeout: float,
        source: str | None,
        source_port: int,
        max_size: bool,
    ...
    async def async_query(
        ...
        source_port: int,
        max_size: bool,
        backend: lambda.vendor.dns.asyncbackend.Backend,
        one_rr_per_rrset: bool = False,
        ignore_trailing: bool = False,
    ) -> dns.message.Message:
        raise NotImplementedError
...
class Do53Nameserver(AddressAndPortNameserver):
    ...
    async def async_query(
        ...
        if max_size:
            response = await lambda.vendor.dns.asyncquery.tcp(
                request,
                self.address,
                timeout=timeout,
                port=self.port,
            ...
            response = await lambda.vendor.dns.asyncquery.udp(
                request,
                self.address,
                timeout=timeout,
                port=self.port,

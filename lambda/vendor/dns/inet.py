...
def inet_pton(family: int, text: str) -> bytes:
    ...
    *text* is a ``str``, the textual address.
    ...
    if family == AF_INET:
        return lambda.vendor.dns.ipv4.inet_aton(text)
    elif family == AF_INET6:
        return lambda.vendor.dns.ipv6.inet_aton(text, True)
    else:
...
def inet_ntop(family: int, address: bytes) -> str:
    ...
    *address* is a ``bytes``, the network address in binary form.
    ...
    if family == AF_INET:
        return lambda.vendor.dns.ipv4.inet_ntoa(address)
    elif family == AF_INET6:
        return lambda.vendor.dns.ipv6.inet_ntoa(address)
    else:
        raise NotImplementedError
...
def af_for_address(text: str) -> int:
    ...
    *text*, a ``str``, the textual address.
    ...
    try:
        lambda.vendor.dns.ipv4.inet_aton(text)
        return AF_INET
    except Exception:
        try:
            lambda.vendor.dns.ipv6.inet_aton(text, True)
            return AF_INET6
        except Exception:
            raise ValueError
...
def is_multicast(text: str) -> bool:
    ...
    *text*, a ``str``, the textual address.
    ...
    try:
        first = lambda.vendor.dns.ipv4.inet_aton(text)[0]
        return first >= 224 and first <= 239
    except Exception:
        try:
            first = lambda.vendor.dns.ipv6.inet_aton(text, True)[0]
            return first == 255
        except Exception:
            raise ValueError
...
def canonicalize(text: str) -> str:
    ...
    *text*, a ``str``, the address in textual form.
    ...
    """
    try:
        return lambda.vendor.dns.ipv6.canonicalize(text)
    except Exception:
        try:
            return lambda.vendor.dns.ipv4.canonicalize(text)
        except Exception:
            raise ValueError

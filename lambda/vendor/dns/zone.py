...
import lambda.vendor.dns.rdataset
import lambda.vendor.dns.rdatatype
import lambda.vendor.dns.rdtypes.ANY.SOA
import lambda.vendor.dns.rdtypes.ANY.ZONEMD
...
import lambda.vendor.dns.zonefile
from lambda.vendor.dns.zonetypes import DigestHashAlgorithm, DigestScheme, _digest_hashers


class BadZone(lambda.vendor.dns.exception.DNSException):
...
class UnsupportedDigestScheme(lambda.vendor.dns.exception.DNSException):
    """The zone digest's scheme is unsupported."""


class UnsupportedDigestHashAlgorithm(lambda.vendor.dns.exception.DNSException):
    """The zone digest's origin is unsupported."""


class NoDigest(lambda.vendor.dns.exception.DNSException):
    """The DNS zone has no ZONEMD RRset at its origin."""


class DigestVerificationFailure(lambda.vendor.dns.exception.DNSException):
    """The ZONEMD digest failed to verify."""


def _validate_name(
    name: lambda.vendor.dns.name.Name,
    origin: lambda.vendor.dns.name.Name | None,
    relativize: bool,
    ...
    if name.is_absolute():
        ...
        except lambda.vendor.dns.name.NameTooLong:
...
class Zone(dns.transaction.TransactionManager):
    ...
    map_factory: Callable[[], MutableMapping[lambda.vendor.dns.name.Name, dns.node.Node]] = dict
    ...
    def __init__(
        self,
        origin: lambda.vendor.dns.name.Name | str | None,
        ...
        *relativize*, a ``bool``, determine's whether domain names are
        ...
        if origin is not None:
            if isinstance(origin, str):
                origin = lambda.vendor.dns.name.from_text(origin)
            elif not isinstance(origin, lambda.vendor.dns.name.Name):
                raise ValueError("origin parameter must be convertible to a DNS name")
            if not origin.is_absolute():
                raise ValueError("origin parameter must be an absolute name")
        self.origin = origin
        self.rdclass = rdclass
        self.nodes: MutableMapping[lambda.vendor.dns.name.Name, dns.node.Node] = self.map_factory()
    ...
    def _validate_name(self, name: lambda.vendor.dns.name.Name | str) -> lambda.vendor.dns.name.Name:
        # Note that any changes in this method should have corresponding changes
        # made in the Version _validate_name() method.
        if isinstance(name, str):
            name = lambda.vendor.dns.name.from_text(name, None)
        elif not isinstance(name, lambda.vendor.dns.name.Name):
    ...
    def find_node(
        self, name: lambda.vendor.dns.name.Name | str, create: bool = False
    ) -> dns.node.Node:
    ...
    def delete_node(self, name: lambda.vendor.dns.name.Name | str) -> None:
    ...
    def find_rdataset(
        self,
        name: lambda.vendor.dns.name.Name | str,
    ...
    def replace_rdataset(
        self, name: lambda.vendor.dns.name.Name | str, replacement: dns.rdataset.Rdataset
    ) -> None:
    ...
    def iterate_rdatasets(
        ...
        covers: dns.rdatatype.RdataType | str = dns.rdatatype.NONE,
    ) -> Iterator[Tuple[lambda.vendor.dns.name.Name, dns.rdataset.Rdataset]]:
    ...
    def iterate_rdatas(
        ...
        rdtype: dns.rdatatype.RdataType | str = dns.rdatatype.ANY,
        covers: dns.rdatatype.RdataType | str = dns.rdatatype.NONE,
    ) -> Iterator[Tuple[lambda.vendor.dns.name.Name, int, dns.rdata.Rdata]]:
    ...
    def check_origin(self) -> None:
        ...
        """
        if self.relativize:
            name = lambda.vendor.dns.name.empty
        else:
            assert self.origin is not None
            name = self.origin
    ...
    def get_soa(
        ...
        """
        if self.relativize:
            origin_name = lambda.vendor.dns.name.empty
        else:
    ...
    def origin_information(
        self,
    ) -> Tuple[lambda.vendor.dns.name.Name | None, bool, lambda.vendor.dns.name.Name | None]:
        effective: lambda.vendor.dns.name.Name | None
        if self.relativize:
...
class Version:
    def __init__(
        ...
        zone: Zone,
        id: int,
        nodes: MutableMapping[lambda.vendor.dns.name.Name, dns.node.Node] | None = None,
    ...
    def _validate_name(self, name: lambda.vendor.dns.name.Name) -> lambda.vendor.dns.name.Name:
        return _validate_name(name, self.origin, self.zone.relativize)

    def get_node(self, name: lambda.vendor.dns.name.Name) -> dns.node.Node | None:
        name = self._validate_name(name)
...
class WritableVersion(Version):
    def __init__(self, zone: Zone, replacement: bool = False):
        ...
        self.changed: Set[lambda.vendor.dns.name.Name] = set()

    def _maybe_cow_with_name(
        self, name: lambda.vendor.dns.name.Name
    ) -> Tuple[dns.node.Node, lambda.vendor.dns.name.Name]:
        name = self._validate_name(name)
        node = self.nodes.get(name)
    ...
    def _maybe_cow(self, name: lambda.vendor.dns.name.Name) -> dns.node.Node:
        return self._maybe_cow_with_name(name)[0]

    def delete_node(self, name: lambda.vendor.dns.name.Name) -> None:
        name = self._validate_name(name)
        if name in self.nodes:
            del self.nodes[name]
            self.changed.add(name)
    ...
    def put_rdataset(
        self, name: lambda.vendor.dns.name.Name, rdataset: dns.rdataset.Rdataset
    ) -> None:
...
class ImmutableVersion(Version):
    def __init__(self, version: Version):
        ...
        self.nodes = lambda.vendor.dns.immutable.Dict(
...
def _from_text(
    text: Any,
    origin: lambda.vendor.dns.name.Name | str | None = None,
    rdclass: dns.rdataclass.RdataClass = dns.rdataclass.IN,
    relativize: bool = True,

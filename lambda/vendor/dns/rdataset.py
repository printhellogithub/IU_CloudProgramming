...
import lambda.vendor.dns.set
import lambda.vendor.dns.ttl

# define SimpleSet here for backwards compatibility
SimpleSet = lambda.vendor.dns.set.Set


class DifferingCovers(lambda.vendor.dns.exception.DNSException):
    """An attempt was made to add a DNS SIG/RRSIG whose covered type
    is not the same as that of the other rdatas in the rdataset."""


class IncompatibleTypes(lambda.vendor.dns.exception.DNSException):
    """An attempt was made to add DNS RR data of an incompatible type."""


class Rdataset(lambda.vendor.dns.set.Set):
    """A DNS rdataset."""
    ...
    def __init__(
        ...
        *ttl*, an ``int``, the TTL.
        ...
        self.rdtype: lambda.vendor.dns.rdatatype.RdataType = rdtype
        self.covers: lambda.vendor.dns.rdatatype.RdataType = covers
        self.ttl = ttl
    ...
    def update_ttl(self, ttl: int) -> None:
        ...
        *ttl*, an ``int`` or ``str``.
        """
        ttl = lambda.vendor.dns.ttl.make(ttl)
        if len(self) == 0:
            self.ttl = ttl
        elif ttl < self.ttl:
            self.ttl = ttl
    ...
    def add(  # pyright: ignore
        self, rd: lambda.vendor.dns.rdata.Rdata, ttl: int | None = None
    ) -> None:
        ...
        *ttl*, an ``int``, the TTL.
        ...
        if ttl is not None:
            self.update_ttl(ttl)
        if self.rdtype == lambda.vendor.dns.rdatatype.RRSIG or self.rdtype == lambda.vendor.dns.rdatatype.SIG:
            covers = rd.covers()
            if len(self) == 0 and self.covers == lambda.vendor.dns.rdatatype.NONE:
                self.covers = covers
            elif self.covers != covers:
                raise DifferingCovers
        if lambda.vendor.dns.rdatatype.is_singleton(rd.rdtype) and len(self) > 0:
            self.clear()
        super().add(rd)
    ...
    def __repr__(self):
        if self.covers == 0:
            ...
            ctext = "(" + lambda.vendor.dns.rdatatype.to_text(self.covers) + ")"
        return (
            "<DNS "
            + lambda.vendor.dns.rdataclass.to_text(self.rdclass)
            + " "
    ...
    def to_text(
        self,
        name: lambda.vendor.dns.name.Name | None = None,
        ...
        *want_comments*, a ``bool``.  If ``True``, emit comments for rdata
        ...
        if len(self) == 0:
            ...
            #
            s.write(
                f"{ntext}{pad}{lambda.vendor.dns.rdataclass.to_text(rdclass)} "
            ...
            for rd in self:
                ...
                s.write(
                    ...
                    f"{lambda.vendor.dns.rdataclass.to_text(rdclass)} "
                    f"{dns.rdatatype.to_text(self.rdtype)} "
...
class ImmutableRdataset(Rdataset):  # lgtm[py/missing-equals]
    ...
    def __init__(self, rdataset: Rdataset):
        ...
        self.items = lambda.vendor.dns.immutable.Dict(rdataset.items)

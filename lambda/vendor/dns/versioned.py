...
class UseTransaction(lambda.vendor.dns.exception.DNSException):
...
class Zone(dns.zone.Zone):  # lgtm[py/missing-equals]
    ...
    node_factory: Callable[[], lambda.vendor.dns.node.Node] = Node
    ...
    def reader(
        ...
        with self._version_lock:
            if id is not None:
                ...
                if self.relativize:
                    oname = lambda.vendor.dns.name.empty
                else:
                ...
                for v in reversed(self._versions):
                    n = v.nodes.get(oname)
                    if n:
                        rds = n.get_rdataset(self.rdclass, lambda.vendor.dns.rdatatype.SOA)
                        if rds is None:
                            continue
                        soa = cast(lambda.vendor.dns.rdtypes.ANY.SOA.SOA, rds[0])

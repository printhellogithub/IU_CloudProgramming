...
import lambda.vendor.botocore.exceptions
import lambda.vendor.botocore.parsers
import lambda.vendor.botocore.serialize
from lambda.vendor.botocore.config import Config
from lambda.vendor.botocore.endpoint import EndpointCreator
from lambda.vendor.botocore.regions import EndpointResolverBuiltins as EPRBuiltins
from lambda.vendor.botocore.regions import EndpointRulesetResolver
from lambda.vendor.botocore.signers import RequestSigner
...
class ClientArgsCreator:
    ...
    def get_client_args(
        ...
        serializer_kwargs = {
            'timestamp_precision': lambda.vendor.botocore.serialize.TIMESTAMP_PRECISION_DEFAULT
        }
        event_emitter.emit(
            event_name,
        ...
        serializer = lambda.vendor.botocore.serialize.create_serializer(
            protocol,
            parameter_validation,
            timestamp_precision=serializer_kwargs['timestamp_precision'],
        )
        response_parser = lambda.vendor.botocore.parsers.create_parser(protocol)
    ...
    def _validate_s3_regional_config(self, config_val):
        if config_val not in VALID_REGIONAL_ENDPOINTS_CONFIG:
            raise lambda.vendor.botocore.exceptions.InvalidS3UsEast1RegionalEndpointConfigError(
    ...
    def _validate_s3_disable_express_session_auth(self, config_val):
        ...
        if not isinstance(config_val, bool) and not string_bool:
            raise lambda.vendor.botocore.exceptions.InvalidConfigError(
                error_msg=(
    ...
    def _get_sts_regional_endpoints_config(self):
        ...
        if (
            ...
            raise lambda.vendor.botocore.exceptions.InvalidSTSRegionalEndpointsConfigError(
    ...
    def _validate_min_compression_size(self, min_size):
        ...
        try:
            ...
            raise lambda.vendor.botocore.exceptions.InvalidConfigError(error_msg=msg)
    ...
    def _handle_checksum_config(
        ...
        if value not in valid_options:
            raise lambda.vendor.botocore.exceptions.InvalidChecksumConfigError(
    ...
    def _compute_account_id_endpoint_mode_config(self, config_kwargs):
        ...
        if signature_version is lambda.vendor.botocore.UNSIGNED:
